...
int main(int argc, char *argv[]) {
    ...
    double memory_bound_factor = 2.0;

    try {

        memory_bound_factor = parser.global_params.get_child("memoryBoundFactor").get_value_optional<double>().value_or(2.0);
    ...
    std::string header = "Graph,Machine,Algorithm,Cost,Async_Cost,Supersteps,TimeToCompute,MemoryBoundFactor";
    ...
    for (const auto &machine_entry : std::filesystem::recursive_directory_iterator(machine_dir)) {
        ...
        for (const auto &graph_entry : std::filesystem::recursive_directory_iterator(graph_dir)) {
            ...
            for (const auto& node : bsp_instance.getComputationalDag().vertices()) {
                bsp_instance.getComputationalDag().setNodeCommunicationWeight(node, bsp_instance.getComputationalDag().nodeMemoryWeight(node));
            }
            ...
            std::vector<unsigned> schedulers_costs(num_algs, 0);
            std::vector<unsigned> schedulers_async_costs(num_algs, 0);
            ...
            for (auto &algorithm : parser.scheduler) {
                ...
                auto [return_status, schedule] = run_algorithm_mem(parser, algorithm.second, bsp_instance, time_limit);
                ...
                schedulers_costs[algorithm_counter] = schedule.computeCost();
                schedulers_async_costs[algorithm_counter] = schedule.computeAsynchronousCost();
                ...
                stats << name_graph << "," 
                      ...
                      << schedulers_costs[algorithm_counter] << ","
                      << schedulers_async_costs[algorithm_counter] << ","
                      << schedulers_supersteps[algorithm_counter] << ","
                      << schedulers_compute_time[algorithm_counter] << ","
                      << memory_bound_factor << std::endl;
                    

                // test rearranging memory policy in the end
                if(algorithm.second.get_child("name").get_value<std::string>() != "GreedyPebbling")
                {
                    BspMemSchedule rearranged = schedule;
                    rearranged.ResetToForesight();

                    stats << name_graph << "," 
                        << name_machine << "," 
                        << schedulers_name[algorithm_counter]+"RtF" << ","
                        << rearranged.computeCost() << ","
                        << rearranged.computeAsynchronousCost() << ","
                        << schedulers_supersteps[algorithm_counter] << ","
                        << schedulers_compute_time[algorithm_counter] << ","
                        << memory_bound_factor << std::endl;
                }
