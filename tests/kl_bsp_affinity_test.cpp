...
BOOST_AUTO_TEST_CASE(SimpleParentChildTest) {
    ...
    auto nodeSelection = kl.InsertGainHeapTest({0});
...
bool ValidateCommDatastructures(const MaxCommDatastructure<Graph, double, KlActiveScheduleT> &commDsIncremental,
    ...
    for (const auto v : instance.Vertices()) {
        for (unsigned p = 0; p < instance.NumberOfProcessors(); ++p) {
            unsigned countInc = 0;
            if (commDsIncremental.nodeLambdaMap.HasProcEntry(v, p)) {
                countInc = commDsIncremental.nodeLambdaMap.GetProcEntry(v, p);
            }

            unsigned countFresh = 0;
            if (commDsFresh.nodeLambdaMap.HasProcEntry(v, p)) {
                countFresh = commDsFresh.nodeLambdaMap.GetProcEntry(v, p);
            }
...
BOOST_AUTO_TEST_CASE(TestUpdateDatastructureAfterMove) {
    ...
    BOOST_CHECK(ValidateCommDatastructures(
        kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_update_datastructure_after_move"));
    BOOST_CHECK_CLOSE(afterRecomputed, afterTracked, 0.00001);
}
...
BOOST_AUTO_TEST_CASE(TestMultipleSequentialMoves) {
    ...
    BOOST_CHECK(ValidateCommDatastructures(
        kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_multiple_sequential_moves_1"));
    ...
    BOOST_CHECK(ValidateCommDatastructures(
        kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_multiple_sequential_moves_2"));
    ...
    BOOST_CHECK(ValidateCommDatastructures(
        kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_multiple_sequential_moves_3"));
...
BOOST_AUTO_TEST_CASE(TestNodeWithMultipleChildren) {
    ...
    kl.InsertGainHeapTest({1});
    kl.GetCommCostF().ComputeScheduleCost();
    kl.RunInnerIterationTest();

    double afterMove1Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove1Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(
        kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_node_with_multiple_children"));
    BOOST_CHECK_CLOSE(afterMove1Recomputed, afterMove1Tracked, 0.00001);
    ...
    double afterMove2Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(
        kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_node_with_multiple_children_2"));
    BOOST_CHECK_CLOSE(afterMove2Recomputed, afterMove2Tracked, 0.00001);
    ...
    double afterMove3Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(
        kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_node_with_multiple_children_3"));
    BOOST_CHECK_CLOSE(afterMove3Recomputed, afterMove3Tracked, 0.00001);
...
BOOST_AUTO_TEST_CASE(TestCrossStepMoves) {
    ...
    double afterMove1Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove1Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "test_cross_step_moves_1"));
    BOOST_CHECK_CLOSE(afterMove1Recomputed, afterMove1Tracked, 0.00001);
}
...
BOOST_AUTO_TEST_CASE(TestComplexScenario) {
    ...
    kl.InsertGainHeapTest({v3, v1});
    kl.RunInnerIterationTest();

    double afterMove1Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove1Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "complex_move1"));
    BOOST_CHECK_CLOSE(afterMove1Recomputed, afterMove1Tracked, 0.00001);
    ...
    double afterMove2Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove2Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "complex_move2"));
    ...
    double afterMove3Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove3Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "complex_move3"));
    BOOST_CHECK_CLOSE(afterMove3Recomputed, afterMove3Tracked, 0.00001);
    ...
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "complex_move4"));
    ...
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "complex_move5"));
...
BOOST_AUTO_TEST_CASE(TestComplexScenarioOnlyCompute) {
    ...
    kl2.InsertGainHeapTest({v2});
    kl2.RunInnerIterationTest();

    BOOST_CHECK(ValidateCommDatastructures(kl2.GetCommCostF().commDs, kl2.GetActiveSchedule(), instance, "complex_move2"));
    BOOST_CHECK_CLOSE(kl2.GetCommCostF().ComputeScheduleCostTest(), kl2.GetCurrentCost(), 0.00001);

    KlImproverTest kl3;
    kl3.SetupSchedule(schedule);

    kl3.InsertGainHeapTest({v3});
    kl3.RunInnerIterationTest();

    BOOST_CHECK(ValidateCommDatastructures(kl3.GetCommCostF().commDs, kl3.GetActiveSchedule(), instance, "complex_move3"));
    BOOST_CHECK_CLOSE(kl3.GetCommCostF().ComputeScheduleCostTest(), kl3.GetCurrentCost(), 0.00001);

    KlImproverTest kl4;
    kl4.SetupSchedule(schedule);

    kl4.InsertGainHeapTest({v4});
    kl4.RunInnerIterationTest();

    BOOST_CHECK(ValidateCommDatastructures(kl4.GetCommCostF().commDs, kl4.GetActiveSchedule(), instance, "complex_move4"));
    BOOST_CHECK_CLOSE(kl4.GetCommCostF().ComputeScheduleCostTest(), kl4.GetCurrentCost(), 0.00001);

    KlImproverTest kl5;
    kl5.SetupSchedule(schedule);

    kl5.InsertGainHeapTest({v5});
    kl5.RunInnerIterationTest();

    BOOST_CHECK(ValidateCommDatastructures(kl5.GetCommCostF().commDs, kl5.GetActiveSchedule(), instance, "complex_move5"));
    BOOST_CHECK_CLOSE(kl5.GetCommCostF().ComputeScheduleCostTest(), kl5.GetCurrentCost(), 0.00001);

    KlImproverTest kl6;
    kl6.SetupSchedule(schedule);

    kl6.InsertGainHeapTest({v6});
    kl6.RunInnerIterationTest();

    BOOST_CHECK(ValidateCommDatastructures(kl6.GetCommCostF().commDs, kl6.GetActiveSchedule(), instance, "complex_move6"));
    BOOST_CHECK_CLOSE(kl6.GetCommCostF().ComputeScheduleCostTest(), kl6.GetCurrentCost(), 0.00001);

    KlImproverTest kl7;
    kl7.SetupSchedule(schedule);

    kl7.InsertGainHeapTest({v7});
    kl7.RunInnerIterationTest();

    BOOST_CHECK(ValidateCommDatastructures(kl7.GetCommCostF().commDs, kl7.GetActiveSchedule(), instance, "complex_move7"));
    BOOST_CHECK_CLOSE(kl7.GetCommCostF().ComputeScheduleCostTest(), kl7.GetCurrentCost(), 0.00001);

    KlImproverTest kl8;
    kl8.SetupSchedule(schedule);

    kl8.InsertGainHeapTest({v8});
    kl8.RunInnerIterationTest();

    BOOST_CHECK(ValidateCommDatastructures(kl8.GetCommCostF().commDs, kl8.GetActiveSchedule(), instance, "complex_move8"));
...
BOOST_AUTO_TEST_CASE(TestGridGraphComplexMoves) {
    ...
    double afterMove1Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove1Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "grid_move1"));
    BOOST_CHECK_CLOSE(afterMove1Recomputed, afterMove1Tracked, 0.00001);
    ...
    double afterMove2Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove2Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "grid_move2"));
    BOOST_CHECK_CLOSE(afterMove2Recomputed, afterMove2Tracked, 0.00001);
    ...
    double afterMove3Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove3Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "grid_move3"));
    BOOST_CHECK_CLOSE(afterMove3Recomputed, afterMove3Tracked, 0.00001);
    ...
    double afterMove4Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove4Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "grid_move4"));
    BOOST_CHECK_CLOSE(afterMove4Recomputed, afterMove4Tracked, 0.00001);
}
...
BOOST_AUTO_TEST_CASE(TestButterflyGraphMoves) {
    ...
    double afterMove1Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove1Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "butterfly_move1"));
    BOOST_CHECK_CLOSE(afterMove1Recomputed, afterMove1Tracked, 0.00001);
    ...
    double afterMove2Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove2Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "butterfly_move2"));
    BOOST_CHECK_CLOSE(afterMove2Recomputed, afterMove2Tracked, 0.00001);
    ...
    double afterMove3Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove3Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "butterfly_move3"));
    BOOST_CHECK_CLOSE(afterMove3Recomputed, afterMove3Tracked, 0.00001);
    ...
    double afterMove4Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove4Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "butterfly_move4"));
    BOOST_CHECK_CLOSE(afterMove4Recomputed, afterMove4Tracked, 0.00001);
}
...
BOOST_AUTO_TEST_CASE(TestLadderGraphMoves) {
    ...
    double afterMove1Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove1Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "ladder_move1"));
    BOOST_CHECK_CLOSE(afterMove1Recomputed, afterMove1Tracked, 0.00001);
    ...
    double afterMove2Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove2Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "ladder_move2"));
    BOOST_CHECK_CLOSE(afterMove2Recomputed, afterMove2Tracked, 0.00001);
    ...
    double afterMove3Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove3Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "ladder_move3"));
    BOOST_CHECK_CLOSE(afterMove3Recomputed, afterMove3Tracked, 0.00001);
    ...
    double afterMove4Recomputed = kl.GetCommCostF().ComputeScheduleCostTest();
    double afterMove4Tracked = kl.GetCurrentCost();
    BOOST_CHECK(ValidateCommDatastructures(kl.GetCommCostF().commDs, kl.GetActiveSchedule(), instance, "ladder_move4"));
    BOOST_CHECK_CLOSE(afterMove4Recomputed, afterMove4Tracked, 0.00001);
}
