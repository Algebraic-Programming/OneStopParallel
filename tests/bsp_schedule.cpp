...
BOOST_AUTO_TEST_CASE(TestScheduleWriter) {
    ...
    BOOST_CHECK_EQUAL(scheduleT2.getInstance().GetComputationalDag().num_vertices(), instance.GetComputationalDag().NumVertices());
    BOOST_CHECK(scheduleT2.SatisfiesPrecedenceConstraints());

    BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().num_vertices(), instance.GetComputationalDag().NumVertices());
    BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().num_vertex_types(), instance.GetComputationalDag().NumVertexTypes());
    ...
    for (const auto &v : instance.GetComputationalDag().Vertices()) {
        BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().vertex_work_weight(v),
                          instance.GetComputationalDag().VertexWorkWeight(v));
        BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().vertex_comm_weight(v),
                          instance.GetComputationalDag().VertexCommWeight(v));

        BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().VertexMemWeight(v), instance.GetComputationalDag().vertex_mem_weight(v));

        BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().vertex_type(v), instance.GetComputationalDag().VertexType(v));

        BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().out_degree(v), instance.GetComputationalDag().OutDegree(v));

        BOOST_CHECK_EQUAL(instanceT2.GetComputationalDag().in_degree(v), instance.GetComputationalDag().InDegree(v));
    }
