...
BOOST_AUTO_TEST_CASE(TestLazyAndBufferedModes) {
    ...
    std::cout << "Start Eager Test" << std::endl;
    {
        ...
        std::cout << "node_lambda_vec size: " << commDs.nodeLambdaMap.nodeLambdaVec.size() << std::endl;
        if (commDs.nodeLambdaMap.nodeLambdaVec.size() > 0) {
            std::cout << "node_lambda_vec[0] size: " << commDs.nodeLambdaMap.nodeLambdaVec[0].size() << std::endl;
        }
