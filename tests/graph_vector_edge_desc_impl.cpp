...
BOOST_AUTO_TEST_CASE(test_dag_edge_idx) {
    ...
    for (const auto &v : graph.vertices()) {
        ...
        i = 0;
        for (const auto &e : graph.out_edges(v)) { BOOST_CHECK_EQUAL(e.target, out_neighbors[v][i++]); }
        ...
        i = 0;
        for (const auto &e : graph.in_edges(v)) { BOOST_CHECK_EQUAL(e.source, in_neighbors[v][i++]); }
