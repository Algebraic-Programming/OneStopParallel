...
BOOST_AUTO_TEST_CASE(Graph1KeepOrder) {
    ...
    for (const auto &vert : graph.Vertices()) {
        ...
        cntr = 0;
        for (const auto &e : osp::OutEdges(vert, graph)) {
    ...
    for (const auto &vert : graph.Vertices()) {
        ...
        cntr = 0;
        for (const auto &e : osp::InEdges(vert, graph)) {
...
BOOST_AUTO_TEST_CASE(GraphEdgeContruction) {
    ...
    CompactSparseGraph<true, false, false, false, false, VertexIdx> copyGraph(graph.num_vertices(), edge_view(graph));
    ...
    for (const auto &vert : copyGraph.Vertices()) {
        BOOST_CHECK_EQUAL(copyGraph.OutDegree(vert), outEdges[vert].size());
        std::size_t cntr = 0;
    ...
    CompactSparseGraph<false, false, false, false, false, VertexIdx> reorderGraph(graph.num_vertices(), edge_view(graph));
    ...
    for (const auto &vert : reorderGraph.Vertices()) {
        BOOST_CHECK_EQUAL(reorderGraph.OutDegree(vert), outEdges[graphPerm[vert]].size());
        std::size_t oriVert = graphPerm[vert];
...
BOOST_AUTO_TEST_CASE(GraphTypeCopyContruction) {
    ...
    for (const auto &vert : copyGraph.Vertices()) {
        BOOST_CHECK_EQUAL(graph.vertex_work_weight(vert), copyGraph.VertexWorkWeight(vert));
        BOOST_CHECK_EQUAL(graph.vertex_comm_weight(vert), copyGraph.VertexCommWeight(vert));
        BOOST_CHECK_EQUAL(graph.vertex_mem_weight(vert), copyGraph.VertexMemWeight(vert));
        BOOST_CHECK_EQUAL(graph.vertex_type(vert), copyGraph.VertexType(vert));
    }
    ...
    for (const auto &vert : reorderGraph.Vertices()) {
        BOOST_CHECK_EQUAL(graph.vertex_work_weight(graphPerm[vert]), reorderGraph.VertexWorkWeight(vert));
        BOOST_CHECK_EQUAL(graph.vertex_comm_weight(graphPerm[vert]), reorderGraph.VertexCommWeight(vert));
        BOOST_CHECK_EQUAL(graph.vertex_mem_weight(graphPerm[vert]), reorderGraph.VertexMemWeight(vert));
        BOOST_CHECK_EQUAL(graph.vertex_type(graphPerm[vert]), reorderGraph.VertexType(vert));
    }
