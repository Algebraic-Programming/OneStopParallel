...
std::pair<RETURN_STATUS, BspSchedule> Serial::computeSchedule(const BspInstance &instance) {
  BspSchedule sched(instance);

  for (unsigned vert = 0; vert < instance.numberOfVertices(); vert++) {
    sched.setAssignedSuperstep(vert, 0);
    sched.setAssignedProcessor(vert, 0);
  }
