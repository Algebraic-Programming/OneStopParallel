...
std::pair<RETURN_STATUS, SmSchedule> SMFunGrowlv2::computeSmSchedule(const SmInstance &instance) const {
...
void SMFunGrowlv2::computeTransitiveReduction(const SmInstance &instance, bool *const transEdgeMask, const unsigned int startNode, const unsigned int endNode) const {
#ifdef TIME_THREADS_FUN_GROWL_V2
...
void SMFunGrowlv2::computePartialSchedule(const SmInstance &instance, const bool *const transEdgeMask, unsigned *const node_to_proc, unsigned *const node_to_supstep,  const unsigned int startNode, const unsigned int endNode, unsigned &supstep) const {
    ...
    // Funnel
    const unsigned maxWeightFunnel = static_cast<unsigned>( params.maxWeightAverageMultiplier * (static_cast<double>((*csr_mat).nonZeros()) / static_cast<double>((*csr_mat).rows())) );
    ...
    for (unsigned baseVert = std::max(endNode, 1U) - 1; baseVert >= finalFunnelNode; baseVert--) {
        ...
        std::set<unsigned, std::greater<unsigned>> funnelQueue;
        funnelQueue.insert(baseVert);

        while (!funnelQueue.empty()) {
            const unsigned vert = *funnelQueue.begin();
            funnelQueue.erase(funnelQueue.begin());
...
void SMFunGrowlv2::incrementScheduleSupersteps(unsigned *const node_to_supstep, const unsigned startNode, const unsigned endNode, const unsigned incr) const {
    for (unsigned node = startNode; node < endNode; node++) {
        node_to_supstep[node] += incr;
    }
}

std::pair<RETURN_STATUS, SmSchedule> SMFunGrowlv2::computeScheduleParallel(const SmInstance &instance, unsigned numThreads) const {
