...
double kl_total_cut::compute_current_costs() {
    ...
    for (const auto &edge : current_schedule.instance->getComputationalDag().edges()) {
        ...
        const unsigned &target = current_schedule.instance->getComputationalDag().target(edge);
...
void kl_total_cut::compute_comm_gain(unsigned node, unsigned current_step, unsigned current_proc, unsigned new_proc) {
    ...
    if (current_schedule.use_node_communication_costs) {
        ...
        if (current_proc == new_proc) {
            ...
            for (const auto &target : current_schedule.instance->getComputationalDag().children(node)) {
                ...
                const double loss =
                    (double)current_schedule.instance->getComputationalDag().nodeCommunicationWeight(node) * 1.0 *
        ...
        if (current_proc == new_proc) {
            ...
            for (const auto &out_edge : current_schedule.instance->getComputationalDag().out_edges(node)) {
                ...
                const double loss =
                    (double)current_schedule.instance->getComputationalDag().edgeCommunicationWeight(out_edge) * 1.0 *
                    current_schedule.comm_multiplier;
            ...
            for (const auto &in_edge : current_schedule.instance->getComputationalDag().in_edges(node)) {
                ...
                const double loss =
                    (double)current_schedule.instance->getComputationalDag().edgeCommunicationWeight(in_edge) * 1.0 *
                    current_schedule.comm_multiplier;
