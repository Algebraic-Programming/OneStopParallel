...
#include "scheduler/Coarsers/WorkNoTalk.hpp"
...
RETURN_STATUS WorkNoTalk::run_contractions() {
    ...
    if (getOriginalInstance() != nullptr) {
        comm_cost_multiplier = 0;
        for (size_t p1 = 0; p1 < getOriginalInstance()->numberOfProcessors(); p1++) {
            for (size_t p2 = 0; p2 < getOriginalInstance()->numberOfProcessors(); p2++) {
                if (p1 != p2) {
                    comm_cost_multiplier += getOriginalInstance()->communicationCosts(p1, p2);
                }
            }
        }
        comm_cost_multiplier /= (std::pow(getOriginalInstance()->numberOfProcessors(),2));
    } else {
