...
#include "scheduler/SchedulePermutations/cuthill_mckee.hpp"
...
struct cm_vertex {
    ...
    cm_vertex(unsigned vertex_, int degree_, unsigned parent_position_)
        : vertex(vertex_), degree(degree_), parent_position(parent_position_) {}
...
std::vector<unsigned> cuthill_mckee_wavefront(const ComputationalDag &dag, bool perm) {
...
std::vector<unsigned> cuthill_mckee_undirected(const ComputationalDag &dag, bool start_at_sink, bool perm) {

    int (*calc_degree)(unsigned in_degree, unsigned out_degree) = sum;

    std::vector<unsigned> cm_order(dag.numberOfVertices());
...
std::vector<unsigned> top_sort_priority(const ComputationalDag &dag, const std::vector<unsigned> &node_priority) {
