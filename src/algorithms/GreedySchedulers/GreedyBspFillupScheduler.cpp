...
std::pair<RETURN_STATUS, BspSchedule> GreedyBspFillupScheduler::computeScheduleNoHeap(const BspInstance &instance) {
...
bool GreedyBspFillupScheduler::CanChooseNode(const BspInstance &instance, const std::set<int> &allReady,
...
void GreedyBspFillupScheduler::Choose(const BspInstance &instance, const std::vector<std::vector<int>> &procInHyperedge,
...
std::pair<RETURN_STATUS, BspSchedule> GreedyBspFillupScheduler::computeSchedule(const BspInstance &instance) {
    ...
    while (!ready.empty() || !finishTimes.empty()) {
        ...
        while (CanChooseNodeHeap(instance, allReady, procReady, procFree)) {
            ...
            ChooseHeap(instance, procInHyperedge, allReady, procReady, procFree, nextNode, nextProc, endSupStep, max_finish_time - time);
...
void GreedyBspFillupScheduler::ChooseHeap(const BspInstance &instance, const std::vector<std::vector<bool>> &procInHyperedge,
                                    std::set<VertexType> &allReady,
...
// auxiliary - check if it is possible to assign a node at all
bool GreedyBspFillupScheduler::CanChooseNodeHeap(const BspInstance &instance, const std::set<VertexType> &allReady,
