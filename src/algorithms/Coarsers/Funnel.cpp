...
void Funnel::expand_in_group_dfs(const std::unordered_set<EdgeType, EdgeType_hash>& edge_mask, std::unordered_set<VertexType>& group, std::unordered_map<VertexType, unsigned>& children_not_in_group, long unsigned& group_weight, const double& max_weight, const VertexType active_node, bool& failed_to_add) {
    ...
    for (const EdgeType in_edge : graph.in_edges(active_node)) {
        ...
        if (children_not_in_group[par] == 0) {
            if (group_weight + graph.nodeWorkWeight(par) > max_weight) {
                ...
                group.emplace(par);
                expand_in_group_dfs(edge_mask, group, children_not_in_group, group_weight, max_weight, par, failed_to_add);
                if (failed_to_add) return;
            }
        }
    }
...
void Funnel::expand_out_group_dfs(const std::unordered_set<EdgeType, EdgeType_hash>& edge_mask, std::unordered_set<VertexType>& group, std::unordered_map<VertexType, unsigned>& parents_not_in_group, long unsigned& group_weight, const double& max_weight, const VertexType active_node, bool& failed_to_add) {
    if (failed_to_add) return;
    const ComputationalDag& graph = dag_history.back()->getComputationalDag();
    ...
    for (const EdgeType out_edge : graph.out_edges(active_node)) {
        ...
        if (parents_not_in_group[child] == 0) {
            if (group_weight + graph.nodeWorkWeight(child) > max_weight) {
                ...
                group.emplace(child);
                expand_in_group_dfs(edge_mask, group, parents_not_in_group, group_weight, max_weight, child, failed_to_add);
                if (failed_to_add) return;
            }
        }
    }
...
void Funnel::run_in_contraction() {
    ...
    for (auto rev_top_it = top_order.rbegin(); rev_top_it != top_order.crend(); rev_top_it++) {
        ...
        expand_in_group_dfs(edge_mask, group, children_not_in_group, weight_of_group, parameters.max_relative_weight * (double)median, bottom_node, failed_to_add);
...
void Funnel::run_out_contraction() {
    ...
    for (auto top_it = top_order.begin(); top_it != top_order.cend(); top_it++) {
        ...
        expand_out_group_dfs(edge_mask, group, parents_not_in_group, weight_of_group, parameters.max_relative_weight * (double)median, top_node, failed_to_add);
        partition.push_back(group);
