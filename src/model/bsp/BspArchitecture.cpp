...
void osp::BspArchitecture::setSendCosts(const std::vector<std::vector<unsigned>> &vec) {
...
void osp::BspArchitecture::setNumberOfProcessors(unsigned int num_proc) {
...
void osp::BspArchitecture::setProcessorsWithTypes(const std::vector<unsigned> &processor_types_) {
...
void osp::BspArchitecture::set_processors_consequ_types(const std::vector<unsigned> &processor_type_count_,
                                                   const std::vector<unsigned> &processor_type_memory_) {

                                                    
    if(processor_type_count_.size() != processor_type_memory_.size()) {
        throw std::invalid_argument("Invalid Argument, processor_type_count_ and processor_type_memory_ must have the same size");
    }
...
void osp::BspArchitecture::SetUniformSendCost() {
    for (unsigned i = 0; i < number_processors; i++) {
        for (unsigned j = 0; j < number_processors; j++) {
            if (i == j) {
                send_costs[i][j] = 0;
...
void osp::BspArchitecture::SetExpSendCost(const unsigned int base) {
...
unsigned osp::BspArchitecture::computeCommAverage() const {
    double avg = 0;
    for (unsigned i = 0; i < number_processors; ++i)
        for (unsigned j = 0; j < number_processors; ++j)
            avg += send_costs[i][j];
    ...
    return static_cast<unsigned>(std::round(avg));
};

bool osp::BspArchitecture::are_send_cost_numa() {
...
void osp::BspArchitecture::print_architecture(std::ostream &os) const {
...
void osp::BspArchitecture::updateNumberOfProcessorTypes() {
    number_of_processor_types = 0;
...
unsigned osp::BspArchitecture::maxMemoryBoundProcType(unsigned procType) const {
    unsigned max_mem = 0;
