...
// main parameters for running partitioners
struct CommandLineParserPartition {
  public:
  ...
  private:
    ...
    void usage() {
        std::cout << "Usage: Either read config file: \n"
                  ...
                  << "     --output, -o                   \tOutput partition file \n"
  ...
  public:
    CommandLineParserPartition(const int argc, const char *const argv[], std::string main_config_file_)
            ...
            for (int i = 1; i < argc; ++i) {
                ...
                if (std::string(flag) == "--config") {
                    ...
                    global_params.put("timeLimit", std::stoi(argv[++i]));
                           ...
                           std::string(flag) == "-dot") {
                    global_params.put("outputDotPartition", true);
                    ...
                    global_params.put("outputPartition", true);
                } else {
                    add_algorithm(flag);
                }
            }
