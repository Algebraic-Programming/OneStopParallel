...
using csr_graph = boost::compressed_sparse_row_graph<boost::bidirectionalS, Vertex, Edge>;

using csr_vertex = typename boost::graph_traits<csr_graph>::vertex_descriptor;
using csr_edge = typename boost::graph_traits<csr_graph>::edge_descriptor;


// struct csr_edge_hash {

//     std::size_t operator()(const csr_edge &p) const {
...
class BspInstance_csr {
  ...
  public:
     ...
     */
    BspInstance_csr(csr_graph cdag, BspArchitecture architecture) : cdag(cdag), architecture(architecture) {}
     ...
     */
    inline const csr_graph &getComputationalDag() const { return cdag; }
     ...
     * @return A reference to the computational DAG for the instance.
     */
    inline csr_graph &getComputationalDag() { return cdag; }


    inline unsigned int numberOfVertices() const { return boost::num_vertices(cdag); }
