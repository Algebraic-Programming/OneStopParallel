...
template<bool keep_vertex_order, bool use_work_weights = false, bool use_comm_weights = false,
         bool use_mem_weights = false, bool use_vert_types = false, typename vert_t = std::size_t,
         typename edge_t = std::size_t, typename work_weight_type = unsigned, typename comm_weight_type = unsigned,
         typename mem_weight_type = unsigned, typename vertex_type_template_type = unsigned>
class Compact_Sparse_Graph {
  ...
  private:
    using ThisT = Compact_Sparse_Graph<keep_vertex_order, use_work_weights, use_comm_weights, use_mem_weights,
                                       use_vert_types, vert_t, edge_t, work_weight_type, comm_weight_type,
                                       mem_weight_type, vertex_type_template_type>;
    ...
    class Compact_Parent_Edges {
      ...
      public:
        ...
        Compact_Parent_Edges(const std::vector<vertex_idx> &csr_edge_parents_,
                             const std::vector<edge_t> &csr_target_ptr_)
        ...
        class Parent_range {
          ...
          public:
            Parent_range(const std::vector<vertex_idx> &csr_edge_parents, const std::vector<edge_t> &csr_target_ptr,
            ...
            inline auto rbegin() const {
                ...
                std::advance(it, _csr_target_ptr[_csr_target_ptr.size() - 1] - 1 - _csr_target_ptr[_vert + 1]);
                return it;
            };
            inline auto rend() const {
                auto it = _csr_edge_parents.crbegin();
                std::advance(it, _csr_target_ptr[_csr_target_ptr.size() - 1] - 1 - _csr_target_ptr[_vert]);
                return it;
            };
        ...
        inline Parent_range parents(const vertex_idx vert) const {
            return Parent_range(csr_edge_parents, csr_target_ptr, vert);
        }
    };
    ...
    class Compact_Children_Edges {
      ...
      public:
        ...
        Compact_Children_Edges(const std::vector<vertex_idx> &csc_edge_children_,
                               const std::vector<edge_t> &csc_source_ptr_)
        ...
        class Children_range {
          ...
          public:
            Children_range(const std::vector<vertex_idx> &csc_edge_children, const std::vector<edge_t> &csc_source_ptr,
                           const vertex_idx vert)
            ...
            inline auto rbegin() const {
                ...
                std::advance(it, _csc_source_ptr[csc_source_ptr.size() - 1] - 1 - _csc_source_ptr[_vert + 1]);
                return it;
            };
            inline auto rend() const {
                auto it = _csc_edge_children.crbegin();
                std::advance(it, _csc_source_ptr[csc_source_ptr.size() - 1] - 1 - _csc_source_ptr[_vert]);
                return it;
            };
  ...
  public:
    ...
    Compact_Sparse_Graph(vertex_idx num_vertices_, const edge_list_type &edges)
        ...
        assert((edges.size() < static_cast<size_t>(std::numeric_limits<edge_t>::max())) &&
               "Number of edge must be strictly smaller than the maximally representable number.");

        if constexpr (std::is_same<edge_list_type, std::vector<std::pair<vertex_idx, vertex_idx>>>::value) {
            // TODO assert triggers with -g and without -O3 compiler flags
            assert(std::all_of(edges.cbegin(), edges.cend(),
                               [num_vertices_](const std::pair<vertex_idx, vertex_idx> &edge) {
        ...
        if constexpr (keep_vertex_order) {
            assert(
                std::all_of(edges.cbegin(), edges.cend(),
                            [](const std::pair<vertex_idx, vertex_idx> &edge) { return edge.first < edge.second; }) &&
        ...
        if constexpr (keep_vertex_order) {
            ...
            // Generating modified Gorder topological order cf. "Speedup Graph Processing by Graph Ordering" by Hao Wei,
            // Jeffrey Xu Yu, Can Lu, and Xuemin Lin
            ...
            auto v_cmp = [&priorities, &children_tmp](const vertex_idx &lhs, const vertex_idx &rhs) {
                return (priorities[lhs] < priorities[rhs]) ||
                       ((priorities[lhs] == priorities[rhs]) &&
                        (children_tmp[lhs].size() < children_tmp[rhs].size())) ||
                       ((priorities[lhs] == priorities[rhs]) &&
                        (children_tmp[lhs].size() == children_tmp[rhs].size()) && (lhs > rhs));
            };
            ...
            for (vertex_idx new_pos = 0; new_pos < num_vertices(); ++new_pos) {
                vertex_permutation_from_original_to_internal[vertex_permutation_from_internal_to_original[new_pos]] =
                    new_pos;
            }
    ...
    template<typename edge_list_type>
    Compact_Sparse_Graph(vertex_idx num_vertices_, edge_list_type &&edges,
                         const std::vector<vertex_work_weight_type> &ww)
        : Compact_Sparse_Graph(num_vertices_, edges) {
        static_assert(use_work_weights, "To set work weight, graph type must allow work weights.");
        assert((ww.size() == static_cast<std::size_t>(num_vertices())) &&
    ...
    template<typename edge_list_type>
    Compact_Sparse_Graph(vertex_idx num_vertices_, const edge_list_type &edges,
                         const std::vector<vertex_work_weight_type> &ww, const std::vector<vertex_comm_weight_type> &cw)
        : Compact_Sparse_Graph(num_vertices_, edges) {
        static_assert(use_work_weights, "To set work weight, graph type must allow work weights.");
    ...
    template<typename edge_list_type>
    Compact_Sparse_Graph(vertex_idx num_vertices_, const edge_list_type &edges,
                         std::vector<vertex_work_weight_type> &&ww, std::vector<vertex_comm_weight_type> &&cw)
        : Compact_Sparse_Graph(num_vertices_, edges) {
        static_assert(use_work_weights, "To set work weight, graph type must allow work weights.");
        static_assert(use_comm_weights, "To set communication weight, graph type must allow communication weights.");
        assert((ww.size() == static_cast<std::size_t>(num_vertices())) &&
    ...
    Compact_Sparse_Graph(vertex_idx num_vertices_, const edge_list_type &edges,
                         const std::vector<vertex_work_weight_type> &ww, const std::vector<vertex_comm_weight_type> &cw,
                         const std::vector<vertex_mem_weight_type> &mw, const std::vector<vertex_type_type> &vt)
        : Compact_Sparse_Graph(num_vertices_, edges) {
        static_assert(use_work_weights, "To set work weight, graph type must allow work weights.");
        static_assert(use_comm_weights, "To set communication weight, graph type must allow communication weights.");
        static_assert(use_mem_weights, "To set memory weight, graph type must allow memory weights.");
    ...
    Compact_Sparse_Graph(vertex_idx num_vertices_, const edge_list_type &edges,
                         std::vector<vertex_work_weight_type> &&ww, std::vector<vertex_comm_weight_type> &&cw,
                         std::vector<vertex_mem_weight_type> &&mw, std::vector<vertex_type_type> &&vt)
        : Compact_Sparse_Graph(num_vertices_, edges) {
        static_assert(use_work_weights, "To set work weight, graph type must allow work weights.");
        static_assert(use_comm_weights, "To set communication weight, graph type must allow communication weights.");
        static_assert(use_mem_weights, "To set memory weight, graph type must allow memory weights.");
    ...
    Compact_Sparse_Graph(const Graph_type &graph, void *)
        : Compact_Sparse_Graph(graph.num_vertices(), edge_view(graph)) {
    ...
    template<typename RetT = void>
    inline std::enable_if_t<not use_work_weights, RetT>
    set_vertex_work_weight(const vertex_idx v, const vertex_work_weight_type work_weight) {
        static_assert(use_work_weights, "To set work weight, graph type must allow work weights.");
    };
    ...
    template<typename RetT = void>
    inline std::enable_if_t<not use_comm_weights, RetT>
    set_vertex_comm_weight(const vertex_idx v, const vertex_comm_weight_type comm_weight) {
    ...
    template<typename RetT = void>
    inline std::enable_if_t<use_vert_types, RetT> set_vertex_type(const vertex_idx v,
                                                                  const vertex_type_type vertex_type_) {
        if constexpr (keep_vertex_order) {
    ...
    template<typename RetT = void>
    inline std::enable_if_t<not use_vert_types, RetT> set_vertex_type(const vertex_idx v,
                                                                      const vertex_type_type vertex_type_) {
