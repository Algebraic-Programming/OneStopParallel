...
/**
 * @brief The GreedySspLocking class represents a scheduler that uses a greedy algorithm to compute schedules for
 * BspInstance.
 *
...
class GreedySspLocking : public SSPScheduler {
  ...
  private:
    ...
    // int computeLockingPenalty(const ComputationalDag &graph);
    ...
    std::set<int> locked_set;
    std::vector<int> locked;
    std::vector<unsigned> locked_superstep;
    int lock_penalty = 1;
    ...
    void computeDefaultScores(const ComputationalDag &graph, const unsigned staleness);

    std::pair<int, double> computeScore(VertexType node, unsigned proc,
                                        const BspInstance &instance, const unsigned supstepIdx,
    ...
    bool Choose(const BspInstance &instance, std::vector<std::set<VertexType>> &allReady, std::vector<std::set<VertexType>> &procReady,
  ...
  public:
    /**
     * @brief Default constructor for GreedySspLocking.
     */
    GreedySspLocking(float max_percent_idle_processors_ = 0.4, bool increase_parallelism_in_new_superstep_ = true)
        : SSPScheduler(), max_percent_idle_processors(max_percent_idle_processors_),
          increase_parallelism_in_new_superstep(increase_parallelism_in_new_superstep_) {}

    /**
     * @brief Default destructor for GreedySspLocking.
     */
    virtual ~GreedySspLocking() = default;
     ...
     */
    virtual std::pair<RETURN_STATUS, SspSchedule> computeSspSchedule(const BspInstance &instance, unsigned staleness) override;
    ...
    virtual std::string getScheduleName() const override {
        ...
        if (use_memory_constraint) {
            return "BspGreedyLockingSSPMemory";
        } else {
            return "BspGreedyLockingSSP_" + std::to_string(max_percent_idle_processors);
        }
    }
