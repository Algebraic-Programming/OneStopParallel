 ...
 * @return std::vector<size_t> vec[prev_node_name] = new_node_name(location)
 */
std::vector<size_t> schedule_node_permuter_small_relation(const BspSchedule& sched, const SCHEDULE_NODE_PERMUTATION_MODES mode = LOOP_PROCESSORS);
 ...
 * @return std::vector<size_t> vec[prev_node_name] = new_node_name(location)
 */
std::vector<size_t> schedule_node_permuter_dfs(const BspSchedule& sched, const SCHEDULE_NODE_PERMUTATION_MODES mode = LOOP_PROCESSORS);
