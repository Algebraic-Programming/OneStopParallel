...
class CoptDirectSendingFullScheduler : public Scheduler {
      ...
      public:
        WriteSolutionCallback()
            : counter(0), max_number_solution(100), best_obj(COPT_INFINITY), write_solutions_path_cb(""),
              ...
              comm_processor_to_processor_superstep_node_var_ptr(0) {}
  ...
  public:
    CoptDirectSendingFullScheduler(unsigned steps = 5)
        : Scheduler(), model(COPTEnv::getInstance().CreateModel("BspSchedule")), use_initial_schedule(false),
          initial_schedule(0), write_solutions_found(false), solution_callback(), max_number_supersteps(steps) {
    ...
    /// @brief Constructs a CoptFullScheduler. The provided schedule is
    /// used to set an initial solution for the ILP. Max number of supersteps is set to schedule.numberOfSupersteps().
    /// @param schedule
    CoptDirectSendingFullScheduler(const BspSchedule &schedule)
        : Scheduler(), model(COPTEnv::getInstance().CreateModel("BspSchedule")), use_initial_schedule(true),
          initial_schedule(&schedule), write_solutions_found(false), solution_callback(),
    ...
    virtual ~CoptDirectSendingFullScheduler() = default;
     ...
     */
    virtual std::string getScheduleName() const override { return "DirectSendingFullIlp"; }
};
