...
class SetSchedule : public IBspSchedule<GraphT> {
  ...
  public:
    ...
    std::vector<std::vector<std::unordered_set<VertexIdx>>> stepProcessorVertices;
    ...
    SetSchedule(const IBspSchedule<GraphT> &schedule)
        ...
        for (const auto v : schedule.GetInstance().Vertices()) {
            stepProcessorVertices[schedule.AssignedSuperstep(v)][schedule.AssignedProcessor(v)].insert(v);
        }
    }
    ...
    void SetAssignedSuperstep(VertexIdx node, unsigned superstep) override {
        ...
        for (unsigned proc = 0; proc < instance_->NumberOfProcessors(); proc++) {
            for (unsigned step = 0; step < numberOfSupersteps; step++) {
                if (stepProcessorVertices[step][proc].find(node) != stepProcessorVertices[step][proc].end()) {
                    assignedProcessor = proc;
                    stepProcessorVertices[step][proc].erase(node);
                }
            }
        }

        stepProcessorVertices[superstep][assignedProcessor].insert(node);
    }
    ...
    void SetAssignedProcessor(VertexIdx node, unsigned processor) override {
        ...
        stepProcessorVertices[assignedStep][processor].insert(node);
    }
