      ...
      public:
        WriteSolutionCallback()
            : counter_(0),
              ...
              allowRecomputationCb(false),
              writeSolutionsPathCb(""),
              solutionFilePrefixCb(""),
              instancePtr(),
              nodeToProcessorSuperstepVarPtr(),
              commProcessorToProcessorSuperstepNodeVarPtr() {}

        bool allowRecomputationCb;
        std::string writeSolutionsPathCb;
        std::string solutionFilePrefixCb;
        const BspInstance<GraphT> *instancePtr;

        std::vector<std::vector<VarArray>> *nodeToProcessorSuperstepVarPtr;
        std::vector<std::vector<std::vector<VarArray>>> *commProcessorToProcessorSuperstepNodeVarPtr;
            ...
            if (Where() == COPT_CBCONTEXT_MIPSOL && counter_ < maxNumberSolution_ && GetIntInfo(COPT_CBINFO_HASINCUMBENT)) {
                try {
                    if (GetDblInfo(COPT_CBINFO_BESTOBJ) < bestObj_ && 0.0 < GetDblInfo(COPT_CBINFO_BESTBND)) {
                        ...
                        if (allowRecomputationCb) {
                            auto sched = ConstructBspScheduleRecompFromCallback();
                            DotFileWriter schedWriter;
                            schedWriter.WriteScheduleRecomp(writeSolutionsPathCb + "intmed_sol_" + solutionFilePrefixCb + "_"
                            ...
                            DotFileWriter schedWriter;
                            schedWriter.WriteSchedule(writeSolutionsPathCb + "intmed_sol_" + solutionFilePrefixCb + "_"
                                                          + std::to_string(counter_) + "_schedule.dot",
                                                      sched);
                        }
                        counter_++;
        ...
        BspScheduleCS<GraphT> ConstructBspScheduleFromCallback() {
            BspScheduleCS<GraphT> schedule(*instancePtr);

            for (const auto &node : instancePtr->Vertices()) {
                for (unsigned int processor = 0; processor < instancePtr->NumberOfProcessors(); processor++) {
                    for (unsigned step = 0; step < static_cast<unsigned>((*nodeToProcessorSuperstepVarPtr)[0][0].Size()); step++) {
            ...
            for (const auto &node : instancePtr->Vertices()) {
                for (unsigned int pFrom = 0; pFrom < instancePtr->NumberOfProcessors(); pFrom++) {
                    for (unsigned int pTo = 0; pTo < instancePtr->NumberOfProcessors(); pTo++) {
                        if (pFrom != pTo) {
                            for (int step = 0; step < (*nodeToProcessorSuperstepVarPtr)[0][0].Size(); step++) {
                                if (GetSolution((*commProcessorToProcessorSuperstepNodeVarPtr)[pFrom][pTo][static_cast<unsigned>(
        ...
        BspScheduleRecomp<GraphT> ConstructBspScheduleRecompFromCallback() {
            ...
            for (unsigned int node = 0; node < instancePtr->NumberOfVertices(); node++) {
                for (unsigned int pFrom = 0; pFrom < instancePtr->NumberOfProcessors(); pFrom++) {
                    for (unsigned int pTo = 0; pTo < instancePtr->NumberOfProcessors(); pTo++) {
                        if (pFrom != pTo) {
                            for (unsigned step = 0; step < static_cast<unsigned>((*nodeToProcessorSuperstepVarPtr)[0][0].Size());
                                 step++) {
                                if (GetSolution(
                                        (*commProcessorToProcessorSuperstepNodeVarPtr)[pFrom][pTo][step][static_cast<int>(node)])
                                    >= .99) {
  ...
  public:
    ...
    virtual void ComputeScheduleBase(const BspScheduleRecomp<GraphT> &schedule, Model &model) {
        ...
        if (writeSolutionsFound_) {
            ...
            solutionCallback.instancePtr = &schedule.GetInstance();
            solutionCallback.commProcessorToProcessorSuperstepNodeVarPtr = &commProcessorToProcessorSuperstepNodeVar_;
            solutionCallback.nodeToProcessorSuperstepVarPtr = &nodeToProcessorSuperstepVar_;
            solutionCallback.solutionFilePrefixCb = solutionFilePrefix_;
            solutionCallback.writeSolutionsPathCb = writeSolutionsPath_;
            solutionCallback.allowRecomputationCb = allowRecomputation_;
