...
class GreedyVarianceSspScheduler : public MaxBspScheduler<GraphT> {
  ...
  private:
    ...
    constexpr static bool useMemoryConstraint = isMemoryConstraintV<MemoryConstraintT>
                                                or isMemoryConstraintScheduleV<MemoryConstraintT>;

    static_assert(not useMemoryConstraint or std::is_same_v<GraphT, typename MemoryConstraintT::GraphImplT>,
    ...
    std::vector<double> ComputeWorkVariance(const GraphT &graph) const {
        ...
        for (auto rIter = topOrder.rbegin(); rIter != topOrder.crend(); rIter++) {
            ...
            double nodeWeight
                = std::log(static_cast<double>(std::max(graph.vertex_work_weight(*rIter), static_cast<VWorkwT<GraphT>>(1))));
    ...
    void Choose(const BspInstance<GraphT> &instance,
        ...
        for (unsigned i = 0; i < instance.NumberOfProcessors(); ++i) {
            ...
            while (it != procReady[i].end()) {
                if (endSupStep && (remainingTime < instance.GetComputationalDag().vertex_work_weight(it->first))) {
            ...
            while (it != readyList.end()) {
                ...
                if (score > maxScore) {
                    if constexpr (useMemoryConstraint) {
                        ...
                        const auto &compatibleTypes
                            = procTypesCompatibleWithNodeTypeSkipProctype[procType][instance.GetComputationalDag().vertex_type(node)];
  ...
  public:
    ...
    RETURN_STATUS ComputeSspSchedule(BspSchedule<GraphT> &schedule, unsigned stale) {
        ...
        std::vector<unsigned> nrOldReadyNodesPerType(g.num_vertex_types(), 0);
        std::vector<std::vector<unsigned>> nrReadyStaleNodesPerType(stale, std::vector<unsigned>(g.num_vertex_types(), 0));
        ...
        for (VertexType node = 0; node < n; ++node) {
            ...
            if (numParents == 0) {
                ...
                nrReadyStaleNodesPerType[0][g.vertex_type(node)]++;
            }
        }
        ...
        while (!oldReady.empty() || nonemptyReady() || !finishTimes.empty()) {
            if (finishTimes.empty() && endSupStep) {
                ...
                for (unsigned nodeType = 0; nodeType < g.num_vertex_types(); ++nodeType) {
                ...
                if (node != std::numeric_limits<VertexType>::max()) {
                    ...
                    for (const auto &succ : g.Children(node)) {
                        ...
                        if (nrPredecRemain[succ] == 0) {
                            ...
                            nrReadyStaleNodesPerType[supstepIdx % stale][g.vertex_type(succ)]++;
                            ...
                            if (instance.IsCompatible(succ, procOfNode)) {
                                ...
                                if constexpr (useMemoryConstraint) {
                                    if (earliestAdd == supstepIdx) {
                                        memoryOk = memoryConstraint_.can_add(succ, procOfNode);
                                    }
                                }
            ...
            while (CanChooseNode(instance, allReady, procReady[supstepIdx % stale], procFree)) {
                ...
                if (procReady[supstepIdx % stale][nextProc].find(std::make_pair(nextNode, workVariances[nextNode]))
                    ...
                    nrOldReadyNodesPerType[g.vertex_type(nextNode)]--;
                ...
                if constexpr (useMemoryConstraint) {
                    ...
                    for (const auto &nodePair : procReady[supstepIdx % stale][nextProc]) {
                        if (!memoryConstraint_.can_add(nodePair.first, nextProc)) {
