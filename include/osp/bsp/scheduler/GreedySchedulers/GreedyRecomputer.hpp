...
class GreedyRecomputer {
  ...
  private:
    ...
    void refreshAuxData(const BspScheduleRecomp<Graph_t> &schedule);

    // elementary operations to edit schedule - add/remove step, and update data structures
    void addCommStep(const BspScheduleRecomp<Graph_t> &schedule, const KeyTriple &new_comm, const unsigned step);
    void removeCommStep(const BspScheduleRecomp<Graph_t> &schedule, const KeyTriple &removed_comm, const unsigned step);
    void addRecomputeStep(BspScheduleRecomp<Graph_t> &schedule, const vertex_idx node, const unsigned proc, const unsigned step);
  ...
  public:
    ...
    RETURN_STATUS computeRecompScheduleBasic(BspScheduleCS<Graph_t> &initial_schedule, BspScheduleRecomp<Graph_t> &recomp_schedule);

    RETURN_STATUS computeRecompScheduleAdvanced(BspScheduleCS<Graph_t> &initial_schedule, BspScheduleRecomp<Graph_t> &recomp_schedule);
};

template <typename Graph_t>
RETURN_STATUS GreedyRecomputer<Graph_t>::computeRecompScheduleBasic(BspScheduleCS<Graph_t> &initial_schedule, BspScheduleRecomp<Graph_t> &recomp_schedule)
{
...
template <typename Graph_t>
RETURN_STATUS GreedyRecomputer<Graph_t>::computeRecompScheduleAdvanced(BspScheduleCS<Graph_t> &initial_schedule, BspScheduleRecomp<Graph_t> &recomp_schedule)
{
    recomp_schedule = BspScheduleRecomp<Graph_t>(initial_schedule);
...
template <typename Graph_t>
bool GreedyRecomputer<Graph_t>::greedyImprove(BspScheduleRecomp<Graph_t> &schedule)
{
...
template <typename Graph_t>
bool GreedyRecomputer<Graph_t>::mergeEntireSupersteps(BspScheduleRecomp<Graph_t> &schedule)
{
  bool improved = false;
  refreshAuxData(schedule);
  ...
  for (unsigned step = 0; step < schedule.numberOfSupersteps() - 1; ++step) {
    ...
    for (unsigned next_step = step + 1; next_step < schedule.numberOfSupersteps(); ++next_step) {
      ...
      for (const std::pair<vertex_idx, unsigned> &new_compute : new_work_steps) {
        new_work_cost[new_compute.second] += G.vertex_work_weight(new_compute.first);
      }
      ...
      if (step > 0) {
        ...
        for (const KeyTriple &new_comm : new_comm_steps_before) {
          cost_type comm_cost = G.vertex_comm_weight(std::get<0>(new_comm)) * 
...
template <typename Graph_t>
bool GreedyRecomputer<Graph_t>::recomputeEntireSupersteps(BspScheduleRecomp<Graph_t> &schedule)
{
  bool improved = false;
  refreshAuxData(schedule);
  ...
  for (unsigned step = 0; step < schedule.numberOfSupersteps(); ++step) {
    for (unsigned from_proc = 0; from_proc < schedule.getInstance().numberOfProcessors(); ++from_proc) {
      for (unsigned to_proc = 0; to_proc < schedule.getInstance().numberOfProcessors(); ++to_proc) {
        ...
        for (const vertex_idx node : must_replicate) {
          new_work_cost += G.vertex_work_weight(node);
        }
...
template <typename Graph_t>
bool GreedyRecomputer<Graph_t>::batchRemoveSteps(BspScheduleRecomp<Graph_t> &schedule)
{
  bool improved = false;
  const Graph_t &G = schedule.getInstance().getComputationalDag();
      ...
      for (unsigned proc = 0; proc < schedule.getInstance().numberOfProcessors(); ++proc) {
        for (unsigned send_or_rec = 0; send_or_rec < 2; ++send_or_rec) {
          ...
          for (const KeyTriple &comm_step : *current_comm_steps) {
            ...
            if (G.vertex_comm_weight(node) == 0) {
              continue;
            }
            if (!schedule.getInstance().isCompatible(node, to_proc)) {
              continue;
            ...
            for (unsigned comp_step = first_computable[node][to_proc]; comp_step <= *needed_on_proc[node][to_proc].begin(); ++comp_step) {
              ...
              if (work_cost[to_proc][comp_step] + assigned_extra + G.vertex_work_weight(node) > max_work[comp_step]) {
                increase = work_cost[to_proc][comp_step] + assigned_extra + G.vertex_work_weight(node) - max_work[comp_step];
              }
              if (increase < smallest_increase) {
                smallest_increase = increase;
          ...
          if (smallest_increase < std::numeric_limits<cost_type>::max()) {
            ...
            if (itr == work_increased.end()) {
              work_increased[std::make_pair(to_proc, best_step_target)] = G.vertex_work_weight(node);
            } else {
              itr->second += G.vertex_work_weight(node);
            }
            ...
            cost_type comm_cost = schedule.getInstance().getComputationalDag().vertex_comm_weight(node)
                        * schedule.getInstance().getArchitecture().communicationCosts(from_proc, to_proc);
...
template <typename Graph_t>
void GreedyRecomputer<Graph_t>::refreshAuxData(const BspScheduleRecomp<Graph_t> &schedule)
{
    const vertex_idx N = schedule.getInstance().numberOfVertices();
    const unsigned P = schedule.getInstance().numberOfProcessors();
    const unsigned S = schedule.numberOfSupersteps();
...
template <typename Graph_t>
void GreedyRecomputer<Graph_t>::addRecomputeStep(BspScheduleRecomp<Graph_t> &schedule, const vertex_idx node, const unsigned proc, const unsigned step)
{
  schedule.assignments(node).emplace_back(proc, step);
  nodes_per_proc_and_step[proc][step].push_back(node);
  work_cost[proc][step] += schedule.getInstance().getComputationalDag().vertex_work_weight(node);
  first_present[node][proc] = std::min(first_present[node][proc], step);
...
template <typename Graph_t>
void GreedyRecomputer<Graph_t>::addCommStep(const BspScheduleRecomp<Graph_t> &schedule, const KeyTriple &new_comm, const unsigned step)
{
  comm_steps[step].insert(new_comm);
  cost_type comm_cost = schedule.getInstance().getComputationalDag().vertex_comm_weight(std::get<0>(new_comm))
                        * schedule.getInstance().getArchitecture().communicationCosts(std::get<1>(new_comm), std::get<2>(new_comm));
...
template <typename Graph_t>
void GreedyRecomputer<Graph_t>::removeCommStep(const BspScheduleRecomp<Graph_t> &schedule, const KeyTriple &removed_comm, const unsigned step)
{
  ...
  cost_type comm_cost = schedule.getInstance().getComputationalDag().vertex_comm_weight(std::get<0>(removed_comm))
