...
class CSchedule {
  ...
  public:
    ...
    std::vector<WorkwT> time;   /**< The time at which each task starts. */
    ...
    CSchedule(std::size_t size)
        : proc(std::vector<unsigned>(size, std::numeric_limits<unsigned>::max())), time(std::vector<WorkwT>(size, 0)) {}
    ...
    void ConvertToBspSchedule(const BspInstance<GraphT> &instance,
        ...
        for (const auto &v : instance.Vertices()) {
            bspSchedule.SetAssignedProcessor(v, proc[v]);
        }
