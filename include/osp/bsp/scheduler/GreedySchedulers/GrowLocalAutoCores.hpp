...
struct GrowLocalAutoCoresParams {
    ...
    WeightT syncCostMultiplierMinSuperstepWeight = 1;
    WeightT syncCostMultiplierParallelCheck = 4;
};
...
class GrowLocalAutoCores : public Scheduler<GraphT> {
  private:
    ...
    static_assert(not useMemoryConstraint
  ...
  public:
    ...
    virtual RETURN_STATUS ComputeSchedule(BspSchedule<GraphT> &schedule) override {
        ...
        const VWorkwT<GraphT> minWeightParallelCheck = params_.syncCostMultiplierParallelCheck * instance.SynchronisationCosts();
        const VWorkwT<GraphT> minSuperstepWeight = params_.syncCostMultiplierMinSuperstepWeight * instance.SynchronisationCosts();

        double desiredParallelism = static_cast<double>(p);

        VertexIdx totalAssigned = 0;
        while (totalAssigned < n) {
            unsigned limit = params_.minSuperstepSize;
            double bestScore = 0;
            ...
            while (continueSuperstepAttempts) {
                ...
                VWorkwT<GraphT> weightLimit = 0, totalWeightAssigned = 0;
