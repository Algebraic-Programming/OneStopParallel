...
class LoadBalancerBase : public Scheduler<GraphT> {
  ...
  protected:
    ...
    std::vector<float> ComputeProcessorPrioritiesInterpolation(const std::vector<v_workw_t<Graph_t>> &superstepPartitionWork,
                                                               const std::vector<v_workw_t<Graph_t>> &totalPartitionWork,
                                                               const v_workw_t<Graph_t> &totalWork,
                                                               const BspInstance<GraphT> &instance,
                                                               const float slack = 0.0) {
        v_workw_t<Graph_t> workTillNow = 0;
    ...
    std::vector<unsigned> ComputeProcessorPriority(const std::vector<v_workw_t<Graph_t>> &superstepPartitionWork,
                                                   const std::vector<v_workw_t<Graph_t>> &totalPartitionWork,
                                                   const v_workw_t<Graph_t> &totalWork,
                                                   const BspInstance<GraphT> &instance,
                                                   const float slack = 0.0) {
