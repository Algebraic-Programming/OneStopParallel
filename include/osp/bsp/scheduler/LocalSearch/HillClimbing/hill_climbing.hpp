...
void HillClimbingScheduler<GraphT>::Init() {
    ...
    // Movement options
    canMove_.clear();
    canMove_.resize(numDirections, std::vector<std::vector<bool>>(n, std::vector<bool>(p, false)));
    moveOptions_.clear();
    moveOptions_.resize(numDirections);
    movePointer_.clear();
    movePointer_.resize(numDirections,
...
int HillClimbingScheduler<GraphT>::MoveCostChange(const VertexIdx node, unsigned p, const int where, StepAuxData &changing) {
    ...
    for (auto entry : sentInc) {
        ...
        auto itr = changing.sentChange.find(std::make_pair(eStep, eProc));
        if (itr == changing.sentChange.end()) {
            changing.sentChange.insert({std::make_pair(eStep, eProc), eIncrease});
        } else {
    ...
    for (auto entry : recInc) {
        ...
        const int eIncrease = std::get<2>(entry);
        affectedSteps.insert(eStep);
        auto itr = changing.recChange.find(std::make_pair(eStep, eProc));
        if (itr == changing.recChange.end()) {
            changing.recChange.insert({std::make_pair(eStep, eProc), eIncrease});
        } else {
            itr->second += eIncrease;
        }
    }
    ...
    changing.newCost = static_cast<CostType>(static_cast<int>(cost_) + change);
    return change;
}
...
bool HillClimbingScheduler<GraphT>::Improve() {
    ...
    while (true) {
        ...
        while (nextMove_.second == moveOptions_[static_cast<unsigned>(nextMove_.first)].end()) {
            dir = (nextMove_.first + 1) % numDirections;
            if (dir == startingDir) {
    ...
    if (shrink_ && bestMoveData.canShrink) {
        Init();
    }
...
void HillClimbingScheduler<GraphT>::CreateSupstepLists() {
    ...
    for (VertexIdx node : top_sort_view(g)) {
