...
struct kl_active_schedule_work_datastructures {
    ...
    void apply_move(kl_move_struct<cost_t, vertex_idx_t> move, work_weight_t work_weight) {      
        ...
        if (move.to_step != move.from_step) {
            ...
            std::sort(step_processor_work_[move.to_step].begin(), step_processor_work_[move.to_step].end());
            unsigned pos = 0;
            const work_weight_t max_work_to = step_processor_work_[move.to_step][0].work;
            for (const auto & wp : step_processor_work_[move.to_step]) {
                step_processor_position[move.to_step][wp.proc] = pos++;

                if (wp.work == max_work_to && pos < instance->numberOfProcessors())
                    step_max_work_processor_count[move.to_step] = pos; 
            }

            std::sort(step_processor_work_[move.from_step].begin(), step_processor_work_[move.from_step].end());
            pos = 0;
            const work_weight_t max_work_from = step_processor_work_[move.from_step][0].work;
            for (const auto & wp : step_processor_work_[move.from_step]) {
                step_processor_position[move.from_step][wp.proc] = pos++;

                if (wp.work == max_work_from && pos < instance->numberOfProcessors())
                    step_max_work_processor_count[move.from_step] = pos; 
            }
            ...
            std::sort(step_processor_work_[move.to_step].begin(), step_processor_work_[move.to_step].end());
            unsigned pos = 0;
            const work_weight_t max_work = step_processor_work_[move.to_step][0].work;
            for (const auto & wp : step_processor_work_[move.to_step]) {
                step_processor_position[move.to_step][wp.proc] = pos++;

                if (wp.work == max_work && pos < instance->numberOfProcessors())
                    step_max_work_processor_count[move.from_step] = pos; 
            }
        }
    }
...
class kl_active_schedule {
  ...
  private:
    ...
    cost_t best_cost = 0;
    size_t best_schedule_idx = 0;

  public:
    virtual ~kl_active_schedule() = default;

    inline const BspInstance<Graph_t> & getInstance() { return *instance; }
    inline cost_t get_cost() { return cost; }
    inline cost_t get_best_cost() { return best_cost; }
    inline bool is_feasible() { return feasible; }
    inline const std::unordered_set<EdgeType> &get_current_violations() { return current_violations; }
    inline unsigned num_steps() const { return vector_schedule.numberOfSupersteps(); }
    inline unsigned assigned_processor(VertexType node) { return vector_schedule.assignedProcessor(node); }
    inline unsigned assigned_superstep(VertexType node) { return vector_schedule.assignedSuperstep(node); }
...
#ifdef KL_DEBUG
        if (not feasible)
            std::cout << "Reverted to best schedule with cost: " << best_cost << std::endl;
#endif
    ...
    inline void update_cost(cost_t change_in_cost) {
        ...
        if (cost <= best_cost && feasible) {
            best_cost = cost;
            best_schedule_idx = applied_moves.size();
        }
    }
...
// template<typename Graph_t, typename cost_t, typename MemoryConstraint_t>
// void kl_active_schedule<Graph_t, cost_t, MemoryConstraint_t>::remove_superstep(unsigned step) {
...
// template<typename Graph_t, typename cost_t, typename MemoryConstraint_t>
// void kl_active_schedule<Graph_t, cost_t, MemoryConstraint_t>::reset_superstep(unsigned step) {
...
// template<typename Graph_t, typename cost_t, typename MemoryConstraint_t>
// void kl_active_schedule<Graph_t, cost_t, MemoryConstraint_t>::recompute_neighboring_supersteps(unsigned step) {
//     if (step > 0) {
//         compute_work_memory_datastructures(step - 1, step);
//         if (step < num_steps() - 1) {
//             compute_work_memory_datastructures(step + 1, step + 1);
