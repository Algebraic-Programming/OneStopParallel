...
struct GenericLambdaVectorContainer {
    ...
    std::vector<std::vector<ValueType>> nodeLambdaVec;
    ...
    inline bool HasProcEntry(const VertexIdxT node, const unsigned proc) const {
        return HasEntry::has_entry(nodeLambdaVec[node][proc]);
    }

    inline ValueType &GetProcEntry(const VertexIdxT node, const unsigned proc) { return nodeLambdaVec[node][proc]; }

    inline ValueType GetProcEntry(const VertexIdxT node, const unsigned proc) const { return nodeLambdaVec[node][proc]; }
