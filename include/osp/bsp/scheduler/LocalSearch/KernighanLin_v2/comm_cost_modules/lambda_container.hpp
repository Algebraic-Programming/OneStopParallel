...
struct LambdaMapContainer {
    ...
    std::vector<std::unordered_map<unsigned, unsigned>> nodeLambdaMap;
     ...
     */
    inline void Initialize(const VertexIdxT numVertices, const unsigned) { nodeLambdaMap.resize(numVertices); }

    /**
     * @brief Reset all processor assignments for a specific node.
     * @param node Node index to reset
     */
    inline void ResetNode(const VertexIdxT node) { nodeLambdaMap[node].clear(); }

    /**
     * @brief Clear all data from the container.
     */
    inline void Clear() { nodeLambdaMap.clear(); }
    ...
    inline bool HasProcEntry(const VertexIdxT node, const unsigned proc) const {
        return (nodeLambdaMap[node].find(proc) != nodeLambdaMap[node].end());
    }
     ...
     */
    inline unsigned &GetProcEntry(const VertexIdxT node, const unsigned proc) { return nodeLambdaMap[node][proc]; }
    ...
    inline unsigned GetProcEntry(const VertexIdxT node, const unsigned proc) const {
        ...
        return nodeLambdaMap[node].at(proc);
    }
     ...
     */
    inline unsigned GetProcCount(const VertexIdxT node) const { return static_cast<unsigned>(nodeLambdaMap[node].size()); }
    ...
    inline bool IncreaseProcCount(const VertexIdxT node, const unsigned proc) {
        if (HasProcEntry(node, proc)) {
            nodeLambdaMap[node][proc]++;
            return false;
        } else {
            nodeLambdaMap[node][proc] = 1;
            return true;
        }
    }
    ...
    inline bool DecreaseProcCount(const VertexIdxT node, const unsigned proc) {
        assert(HasProcEntry(node, proc));
        if (nodeLambdaMap[node][proc] == 1) {
            nodeLambdaMap[node].erase(proc);
            return true;
        } else {
            nodeLambdaMap[node][proc]--;
            return false;
     ...
     */
    inline const auto &IterateProcEntries(const VertexIdxT node) { return nodeLambdaMap[node]; }
};
...
struct LambdaVectorContainer {
    ...
    std::vector<std::vector<unsigned>> nodeLambdaVec;
     ...
     * @param node Node index to reset
     */
    inline void ResetNode(const VertexIdxT node) { nodeLambdaVec[node].assign(numProcs, 0); }

    /**
     * @brief Clear all data from the container.
     */
    inline void Clear() { nodeLambdaVec.clear(); }
     ...
     * @return true if the processor has at least one assignment to the node
     */
    inline bool HasProcEntry(const VertexIdxT node, const unsigned proc) const { return nodeLambdaVec[node][proc] > 0; }
     ...
     * @return true if the processor has no assignments to the node
     */
    inline bool HasNoProcEntry(const VertexIdxT node, const unsigned proc) const { return nodeLambdaVec[node][proc] == 0; }
     ...
     */
    inline unsigned &GetProcEntry(const VertexIdxT node, const unsigned proc) { return nodeLambdaVec[node][proc]; }
    ...
    inline unsigned GetProcEntry(const VertexIdxT node, const unsigned proc) const {
        assert(HasProcEntry(node, proc));
        return nodeLambdaVec[node][proc];
    }
     ...
     */
    inline bool IncreaseProcCount(const VertexIdxT node, const unsigned proc) {
        nodeLambdaVec[node][proc]++;
        return nodeLambdaVec[node][proc] == 1;
    }
    ...
    inline bool DecreaseProcCount(const VertexIdxT node, const unsigned proc) {
        assert(HasProcEntry(node, proc));
        nodeLambdaVec[node][proc]--;
        return nodeLambdaVec[node][proc] == 0;
    }
     ...
     */
    inline auto IterateProcEntries(const VertexIdxT node) { return LambdaVectorRange(nodeLambdaVec[node]); }
};
