...
struct DirectedEdge {
    VertexIdxT<GraphT> source;
    VertexIdxT<GraphT> target;

    bool operator==(const DirectedEdge &other) const { return source == other.source && target == other.target; }

    bool operator!=(const DirectedEdge &other) const { return !(*this == other); }

    DirectedEdge() : source(0), target(0) {}
    ...
    DirectedEdge(VertexIdxT<GraphT> src, VertexIdxT<GraphT> tgt) : source(src), target(tgt) {}
};
