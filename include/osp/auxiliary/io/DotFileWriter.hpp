  ...
  private:
    ...
    struct EdgeWriterDot {
        ...
        void operator()(std::ostream &out, const EdgeDescT<GraphT> &i) const {
            out << Source(i, graph) << "->" << Target(i, graph) << " ["
                << "comm_weight=\"" << graph.EdgeCommWeight(i) << "\";"
                << "]";
        }
    };
    ...
    struct VertexWriterDuplicateRecompScheduleDot {
        ...
        void operator()(std::ostream &out, const VertexOrEdge &i) const {
            out << i << " [" << "label=\"" << name[i] << "\";" << "work_weight=\"" << graph.VertexWorkWeight(i) << "\";"
                << "comm_weight=\"" << graph.VertexCommWeight(i) << "\";" << "mem_weight=\"" << graph.VertexMemWeight(i) << "\";"
                << "proc=\"" << nodeToProc[i] << "\";" << "superstep=\"" << nodeToSuperstep[i] << "\";";
    ...
    struct VertexWriterGraphDot {
        ...
        void operator()(std::ostream &out, const VertexIdxT<GraphT> &i) const {
            out << i << " ["
                << "work_weight=\"" << graph.VertexWorkWeight(i) << "\";"
                << "comm_weight=\"" << graph.VertexCommWeight(i) << "\";"
                << "mem_weight=\"" << graph.VertexMemWeight(i) << "\";";

            if constexpr (hasTypedVerticesV<GraphT>) {
                out << "type=\"" << graph.VertexType(i) << "\";";
            }
    ...
    struct ColoredVertexWriterGraphDot {
        ...
        ColoredVertexWriterGraphDot(const GraphT &graph, const ColorContainerT &colors) : graph(graph), colors(colors) {
            ...
            shapeStrings = {"oval", "rect", "hexagon", "parallelogram"};
        }

        void operator()(std::ostream &out, const VertexIdxT<GraphT> &i) const {
            if (i >= static_cast<VertexIdxT<GraphT>>(colors.size())) {
                ...
                const std::string &color = colorStrings[colors[i] % colorStrings.size()];
                out << i << " [style=filled;fillcolor=" << color << ";";
            }

            out << "work_weight=\"" << graph.VertexWorkWeight(i) << "\";"
                << "comm_weight=\"" << graph.VertexCommWeight(i) << "\";"
                << "mem_weight=\"" << graph.VertexMemWeight(i) << "\";";

            if constexpr (hasTypedVerticesV<GraphT>) {
                out << "type=\"" << graph.VertexType(i) << "\";shape=\""
                    << shapeStrings[graph.VertexType(i) % shapeStrings.size()] << "\";";
            }
