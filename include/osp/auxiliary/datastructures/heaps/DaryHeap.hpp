...
class DaryHeap {
  ...
  public:
    ...
    Key pop() {
        ...
        if (heap_.size() > 1) {
            ...
            heap_.pop_back();
            nodeInfo_.at(heap_[0]).position = 0;
            SiftDown(0);
        } else {
            heap_.pop_back();
        }
    ...
    void update(const Key &key, const Value &newValue) {
        ...
        auto &info = it->second;
        const Value oldValue = info.value;

        if (comp_(newValue, oldValue)) {
            info.value = newValue;
            SiftUp(info.position);
        } else if (comp_(oldValue, newValue)) {
            info.value = newValue;
            SiftDown(info.position);
        }
    }
    ...
    void erase(const Key &key) {
        ...
        if (index != lastIndex) {
            ...
            if (index > 0 && comp_(nodeInfo_.at(movedKey).value, nodeInfo_.at(heap_[Parent(index)]).value)) {
                SiftUp(index);
            } else {
                SiftDown(index);
            }
  ...
  private:
    ...
    inline void SwapNodes(size_t i, size_t j) {
        nodeInfo_.at(heap_[i]).position = j;
        nodeInfo_.at(heap_[j]).position = i;
    ...
    void SiftUp(size_t index) {
        ...
        const Value &valueToSift = nodeInfo_.at(keyToSift).value;

        while (index > 0) {
            size_t pIdx = Parent(index);
            if (comp_(valueToSift, nodeInfo_.at(heap_[pIdx]).value)) {
                heap_[index] = std::move(heap_[pIdx]);
                nodeInfo_.at(heap_[index]).position = index;
                index = pIdx;
            } else {
                break;
            }
    ...
    void SiftDown(size_t index) {
        ...
        while (FirstChild(index) < size) {
            ...
            const Value *bestChildValue = &nodeInfo_.at(heap_[bestChildIdx]).value;
            for (size_t i = bestChildIdx + 1; i < lastChildIdx; ++i) {
                const Value &currentChildValue = nodeInfo_.at(heap_[i]).value;
