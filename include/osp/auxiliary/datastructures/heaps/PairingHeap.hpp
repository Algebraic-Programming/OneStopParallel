...
class PairingHeap {
  private:
    struct Node {
        ...
        Node *nextSibling = nullptr;     // Sibling to the right
        Node *prevOrParent = nullptr;    // If leftmost child, parent; otherwise, left sibling.
    };
    ...
    Node *Meld(Node *heap1, Node *heap2) {
        ...
        if (comp_(heap2->value, heap1->value)) {
            std::swap(heap1, heap2);
        }

        // heap2 becomes the new leftmost child of heap1
        heap2->nextSibling = heap1->child;
        if (heap1->child) {
            heap1->child->prevOrParent = heap2;
        }
        heap1->child = heap2;
        heap2->prevOrParent = heap1;
    ...
    void Cut(Node *node) {
        ...
        if (node->prevOrParent->child == node) {    // is leftmost child
            node->prevOrParent->child = node->nextSibling;
        } else {    // is not leftmost child
            node->prevOrParent->nextSibling = node->nextSibling;
        }
        if (node->nextSibling) {
            node->nextSibling->prevOrParent = node->prevOrParent;
        }
        node->nextSibling = nullptr;
        node->prevOrParent = nullptr;
    }
  ...
  public:
    ...
    PairingHeap(const PairingHeap &other) : numElements_(other.numElements_), comp_(other.comp_) {
        ...
        while (head < q.size()) {
            ...
            if (oldParent->child) {
                const Node *oldChild = oldParent->child;

                // First child
                Node *newChild = new Node{oldChild->key, oldChild->value};
                newParent->child = newChild;
                newChild->prevOrParent = newParent;
                nodeMap_[newChild->key] = newChild;
                oldToNew[oldChild] = newChild;
                q.push_back(oldChild);

                // Siblings
                Node *prevNewSibling = newChild;
                while (oldChild->nextSibling) {
                    oldChild = oldChild->nextSibling;
                    newChild = new Node{oldChild->key, oldChild->value};

                    prevNewSibling->nextSibling = newChild;
                    newChild->prevOrParent = prevNewSibling;
    ...
    Key Pop() {
        ...
        Node *oldRoot = root_;
        Key topKey = oldRoot->key;

        root_ = MultipassMerge(oldRoot->child);
    ...
    void Update(const Key &key, const Value &newValue) {
        ...
        Node *node = it->second;
        const Value oldValue = node->value;
        ...
        if (comp_(newValue, oldValue)) {    // Decrease key
            ...
            if (node != root_) {
                Cut(node);
                if (node->child) {
                    root_ = Meld(root_, MultipassMerge(node->child));
                    node->child = nullptr;
                }
                root_ = Meld(root_, node);
            } else {
    ...
    void Erase(const Key &key) {
        ...
        if (nodeToErase->child) {
            root_ = Meld(root_, MultipassMerge(nodeToErase->child));
            nodeToErase->child = nullptr;
        }
    ...
    std::vector<Key> GetTopKeys(size_t limit = 0) const {
        ...
        const Value &topValue = root_->value;
        std::vector<const Node *> q;
        q.push_back(root_);
        size_t head = 0;
        ...
        while (head < q.size()) {
            ...
            if (comp_(topValue, current->value)) {
                continue;
            }

            topKeys.push_back(current->key);
