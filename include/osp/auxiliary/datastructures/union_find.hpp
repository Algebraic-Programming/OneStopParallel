...
struct UnionFindObject {
    ...
    explicit UnionFindObject(const T &name_, IndexT parentIndex_, WorkwT weight_ = 0, MemwT memory_ = 0)
        : name(name_), parentIndex(parentIndex_), weight(weight_), memory(memory_) {
        rank = 1;
    }
...
class UnionFindUniverse {
  private:
    ...
    IndexT FindOrigin(IndexT index) {
        while (index != universe_[index].parentIndex) {
            universe_[index].parentIndex = universe_[universe_[index].parentIndex].parentIndex;
            index = universe_[index].parentIndex;
        }
        return index;
    }
    ...
    int Join(IndexT index, IndexT otherIndex) {
        ...
        if (universe_[index].rank >= universe_[otherIndex].rank) {
            universe_[otherIndex].parentIndex = index;
            ...
            universe_[index].parentIndex = otherIndex;
