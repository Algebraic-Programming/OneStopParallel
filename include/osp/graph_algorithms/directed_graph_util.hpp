...
struct VertexCondIterator {
  ...
  public:
    ...
    VertexCondIterator(const GraphT &graph_, const IteratorT &start) : graph(graph_), currentVertex(start) {
        while (currentVertex != graph.Vertices().end()) {
            // if (cond.eval(graph, *current_vertex)) {
            if (cond(graph, *currentVertex)) {
                break;
            }
            currentVertex++;
        }
    ...
    value_type operator*() const { return currentVertex.operator*(); }
    ...
    inline bool operator==(const VertexCondIterator &other) { return currentVertex == other.currentVertex; };
...
struct TraversalIterator {
    ...
    ContainerWrapper vertexContainer;

    std::unordered_set<VertexIdxT<GraphT>> visited;
  ...
  public:
    ...
    TraversalIterator(const GraphT &graph_, const VertexIdxT<GraphT> &start)
        : graph(graph_), adjIter(graph_), currentVertex(start) {
        if (graph.NumVertices() == start) {
            return;
        }

        visited.insert(start);

        for (const auto &v : adjIter.Iterate(currentVertex)) {
            vertexContainer.Push(v);
            visited.insert(v);
        }
    }
    ...
    TraversalIterator &operator++() {
        ...
        currentVertex = vertexContainer.PopNext();
    ...
    inline bool operator==(const TraversalIterator &other) { return currentVertex == other.currentVertex; };
...
struct ChildIterator {
    ...
    ChildIterator(const GraphT &graph_) : graph(graph_) {}

    inline auto Iterate(const VertexIdxT<GraphT> &v) const { return graph.Children(v); }
};

template <typename GraphT>
struct BfsQueueWrapper {
    std::queue<VertexIdxT<GraphT>> queue;

    void Push(const VertexIdxT<GraphT> &v) { queue.push(v); }
...
struct DfsStackWrapper {
    std::vector<VertexIdxT<GraphT>> stack;

    void Push(const VertexIdxT<GraphT> &v) { stack.push_back(v); }
...
struct ParentsIterator {
    ...
    ParentsIterator(const GraphT &graph_) : graph(graph_) {}

    inline auto Iterate(const VertexIdxT<GraphT> &v) const { return graph.Parents(v); }
};
