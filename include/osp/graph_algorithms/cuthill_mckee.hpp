...
struct CmVertex {
    ...
    bool operator<(CmVertex const &rhs) const {
        return (parentPosition < rhs.parentPosition) || (parentPosition == rhs.parentPosition and degree < rhs.degree)
               || (parentPosition == rhs.parentPosition and degree == rhs.degree and vertex < rhs.vertex);
    }
};
    ...
    while (nodeCounter < dag.NumVertices()) {
        ...
        if (permutation) {
            for (VertexType i = 0; i < static_cast<VertexType>(currentWavefront.size()); i++) {
                result[currentWavefront[i].vertex] = nodeCounter + i;
            }
        } else {
            for (size_t i = 0; i < currentWavefront.size(); i++) {
                result[nodeCounter + i] = currentWavefront[i].vertex;
            }
        }
        ...
        for (VertexType i = 0; i < static_cast<VertexType>(currentWavefront.size()); i++) {
            for (const auto &child : dag.Children(currentWavefront[i].vertex)) {
