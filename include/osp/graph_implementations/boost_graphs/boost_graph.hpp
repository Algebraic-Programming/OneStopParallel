...
struct BoostVertex {
    ...
    VertexMemwT memoryWeight;
    VertexTypeT nodeType;
};
...
class BoostGraph {
  ...
  public:
    ...
    BoostGraph(const GraphT &other) : numberOfVertexTypes_(0) {
        static_assert(osp::isComputationalDagV<GraphT>, "Graph_t must satisfy the is_computation_dag concept");
    ...
    std::pair<boost::detail::edge_desc_impl<boost::bidirectional_tag, std::size_t>, bool> AddEdge(
        const VertexIdx &src, const VertexIdx &tar, EdgeCommwT commWeight = defaultEdgeCommWeight) {
  ...
  private:
    ...
    static constexpr EdgeCommWeightType defaultEdgeCommWeight = 1;
};
...
static_assert(osp::isConstructableCdagEdgeV<BoostGraphIntT>,
