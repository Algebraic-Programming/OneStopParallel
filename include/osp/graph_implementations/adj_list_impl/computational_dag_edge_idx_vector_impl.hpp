...
struct DirectedEdgeDescriptorImpl {
    ...
    DirectedEdgeDescriptorImpl() : idx(0), source(0), target(0) {}
    ...
    DirectedEdgeDescriptorImpl(VertexIdx sourceArg, VertexIdx targetArg, VertexIdx idxArg)
        : idx(idxArg), source(sourceArg), target(targetArg) {}
...
struct CdagEdgeImpl {
    ...
    CdagEdgeImpl(EdgeCommWeightT commWeightArg = 1) : commWeight(commWeightArg) {}

    EdgeCommWeightT commWeight;
};
...
class ComputationalDagEdgeIdxVectorImpl {
  ...
  public:
    ...
    inline EdgeCommWeightType EdgeCommWeight(DirectedEdgeDescriptor e) const { return edges_[e.idx].commWeight; }

    inline VertexWorkWeightType VertexWorkWeight(VertexIdx v) const { return vertices_[v].workWeight; }

    inline VertexCommWeightType VertexCommWeight(VertexIdx v) const { return vertices_[v].commWeight; }

    inline VertexMemWeightType VertexMemWeight(VertexIdx v) const { return vertices_[v].memWeight; }

    inline unsigned NumVertexTypes() const { return numVertexTypes_; }

    inline VertexTypeType VertexType(VertexIdx v) const { return vertices_[v].vertexType; }

    inline VertexIdx Source(const DirectedEdgeDescriptor &e) const { return e.source; }

    inline VertexIdx Target(const DirectedEdgeDescriptor &e) const { return e.target; }
    ...
    inline void SetVertexWorkWeight(VertexIdx v, VertexWorkWeightType workWeight) { vertices_[v].workWeight = workWeight; }

    inline void SetVertexCommWeight(VertexIdx v, VertexCommWeightType commWeight) { vertices_[v].commWeight = commWeight; }

    inline void SetVertexMemWeight(VertexIdx v, VertexMemWeightType memWeight) { vertices_[v].memWeight = memWeight; }

    inline void SetVertexType(VertexIdx v, VertexTypeType vertexType) {
        vertices_[v].vertexType = vertexType;
        numVertexTypes_ = std::max(numVertexTypes_, vertexType + 1);
    }

    inline void SetEdgeCommWeight(DirectedEdgeDescriptor e, EdgeCommWeightType commWeight) {
        edges_[e.idx].commWeight = commWeight;
    }
