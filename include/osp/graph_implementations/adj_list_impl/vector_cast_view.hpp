...
class VectorCastView {
    ...
    struct CastIterator {
        ...
        CastIterator operator+(difference_type n) const { return CastIterator(currentEdge + n); }

        CastIterator operator-(difference_type n) const { return CastIterator(currentEdge - n); }

        difference_type operator-(const CastIterator &other) const { return currentEdge - other.currentEdge; }

        bool operator==(const CastIterator &other) const { return currentEdge == other.currentEdge; }
