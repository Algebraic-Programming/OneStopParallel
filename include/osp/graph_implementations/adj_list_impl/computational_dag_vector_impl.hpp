...
class ComputationalDagVectorImpl {
  public:
    ...
    void SetVertexCommWeight(const VertexIdx v, const VertexCommWeightType commWeight) {
        vertices_.at(v).commWeight = commWeight;
    }

    void SetVertexMemWeight(const VertexIdx v, const VertexMemWeightType memWeight) { vertices_.at(v).memWeight = memWeight; }

    void SetVertexType(const VertexIdx v, const VertexTypeType vertexType) {
        vertices_.at(v).vertexType = vertexType;
        numVertexTypes_ = std::max(numVertexTypes_, vertexType + 1);
    }
 ...
 */
using ComputationalDagVectorImplDefT = ComputationalDagVectorImpl<CdagVertexImplUnsigned>;
...
static_assert(isDirectedGraphEdgeDescV<ComputationalDagVectorImpl<CdagVertexImplUnsigned>>,
              "computational_dag_vector_impl must satisfy the directed_graph_edge_desc concept");

static_assert(hasVertexWeightsV<ComputationalDagVectorImpl<CdagVertexImplUnsigned>>,
              "computational_dag_vector_impl must satisfy the has_vertex_weights concept");

static_assert(isDirectedGraphV<ComputationalDagVectorImpl<CdagVertexImplUnsigned>>,
              "computational_dag_vector_impl must satisfy the directed_graph concept");

static_assert(isComputationalDagTypedVerticesV<ComputationalDagVectorImpl<CdagVertexImplUnsigned>>,
