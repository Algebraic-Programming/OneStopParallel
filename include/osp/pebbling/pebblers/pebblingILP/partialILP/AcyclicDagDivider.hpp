    ...
    while (true) {
        ...
        for (unsigned idx = 0; idx < subDags.size(); ++idx) {
            ...
            if (!ignoreSourcesInSize_) {
                dagRealSize[idx] = static_cast<unsigned>(dag.num_vertices());
                if (dag.num_vertices() > maxPartitionSize_) {
                    dagIsTooLarge[idx] = true;
                    existsTooLarge = true;
                }
            } else {
                for (vertex_idx localId = 0; local_ID < dag.num_vertices(); ++local_ID) {
        ...
        for (unsigned idx = 0; idx < subDags.size(); ++idx) {
            ...
            if (!dag_is_too_large[idx]) {
                ...
                std::vector<bool> isOriginalSource(dag.num_vertices());
                for (vertex_idx localId = 0; local_ID < dag.num_vertices(); ++local_ID) {
                ...
                // update labels
                std::vector<vertex_idx> nodeIdxInNewSubDag(dag.num_vertices());
...
std::vector<unsigned> AcyclicDagDivider<GraphT>::GetTopologicalSplit(const GraphT &g,
                                                                     std::pair<unsigned, unsigned> minAndMax,
                                                                     const std::vector<bool> &isOriginalSource) const {
    std::vector<unsigned> nodeToPart(g.num_vertices());

    std::vector<vertex_idx> topOrder = GetTopOrder(g);
    std::vector<unsigned> topOrderIdx(g.num_vertices());
    for (unsigned idx = 0; idx < g.num_vertices(); ++idx) {
        topOrderIdx[top_order[idx]] = idx;
    }

    std::vector<unsigned> lastNodeIdxInHyperedge(g.num_vertices());
    ...
    unsigned nodesRemaining = static_cast<unsigned>(g.num_vertices());
    if (ignoreSourcesInSize_) {
        nodesRemaining = 0;
        for (unsigned node = 0; node < g.num_vertices(); ++node) {
    ...
    while (nodesRemaining > min_and_max.second) {
        ...
        for (end = index + 1; index < g.num_vertices() && newlyAddedNodes < minAndMax.first; ++end) {
            if (!ignoreSourcesInSize_ || !is_original_source[end]) {
                ++newlyAddedNodes;
            }
        }

        while (end < g.num_vertices() && newlyAddedNodes < minAndMax.second) {
