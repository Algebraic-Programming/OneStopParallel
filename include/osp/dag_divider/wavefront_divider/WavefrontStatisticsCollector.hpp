...
struct WavefrontStatistics {
    ...
    std::vector<VWorkwT<GraphT>> connectedComponentsWeights;
    std::vector<VMemwT<GraphT>> connectedComponentsMemories;
    std::vector<std::vector<VertexType>> connectedComponentsVertices;
};
...
class WavefrontStatisticsCollector {
  ...
  private:
    ...
    void CollectStatsForLevel(WavefrontStatistics<GraphT> &stats, UnionFind &uf) const {
        ...
        stats.connectedComponentsVertices.reserve(components.size());
        stats.connectedComponentsWeights.reserve(components.size());
        stats.connectedComponentsMemories.reserve(components.size());

        for (const auto &comp : components) {
            auto &[vertices, weight, memory] = comp;
            stats.connectedComponentsVertices.emplace_back(vertices);
            stats.connectedComponentsWeights.emplace_back(weight);
            stats.connectedComponentsMemories.emplace_back(memory);
        }
    }
