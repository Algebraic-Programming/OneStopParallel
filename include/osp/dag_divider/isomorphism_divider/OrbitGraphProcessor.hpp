        ...
        while (changed) {
            ...
            for (const auto u : currentCoarseGraph.Vertices()) {
                for (const auto v : currentCoarseGraph.Children(u)) {
                    ...
                    auto [temp_coarse_graph, temp_contraction_map] = SimulateMerge(u, v, currentCoarseGraph);

                    if (CriticalPathWeight(temp_coarse_graph)
    ...
    void ContractEdges(const GraphT &originalDag,
        ...
        while (changed) {
            ...
            for (const auto &edge : Edges(currentCoarseGraph)) {
                ...
                if (CriticalPathWeight(temp_coarse_graph) > (pathThreshold * static_cast<VWorkwT<ConstrGraphT>>(newSubgraphs.size())
                                                             + CriticalPathWeight(currentCoarseGraph))) {
                    nonViableCritPathEdgesCache_.insert({u, v});
                    continue;
                }
  ...
  private:
    ...
    bool IsMergeViable(const GraphT &originalDag,
        ...
        allNodes.reserve(groupU.subgraphs.size() * (groupU.subgraphs.empty() ? 0 : groupU.subgraphs[0].size())
                         + groupV.subgraphs.size() * (groupV.subgraphs.empty() ? 0 : groupV.subgraphs[0].size()));
        for (const auto &sg : groupU.subgraphs) {
