...
class IsomorphicSubgraphScheduler {
  ...
  public:
    ...
    std::vector<VertexIdxT<GraphT>> ComputePartition(const BspInstance<GraphT> &instance) {
        ...
        SubgraphSchedule subgraphSchedule
            = etfScheduler.Run(input.instance, input.multiplicities, input.requiredProcTypes, input.maxNumProcessors);
        subgraphSchedule.wasTrimmed = std::move(wasTrimmed);    // Pass through trimming info
        ...
        if (plotDotGraphs_) {
            ...
            writer.WriteGraph(timestamp + "iso_groups_contracted.dot", input.instance.GetComputationalDag());
            ...
            writer.WriteGraph(timestamp + "block_graph.dot", coraseGraph);
        }
        return partition;
    }
  ...
  protected:
    ...
    struct SubgraphSchedulerInput {
        BspInstance<CGT> instance;
        std::vector<unsigned> multiplicities;
        std::vector<unsigned> maxNumProcessors;
        std::vector<std::vector<VWorkwT<GT>>> requiredProcTypes;
    };
    ...
    void TrimSubgraphGroups(std::vector<typename OrbitGraphProcessor<GraphT, ConstrGraphT>::Group> &isomorphicGroups,
        ...
        for (size_t groupIdx = 0; groupIdx < isomorphicGroups.size(); ++groupIdx) {
            ...
            if (gcd < groupSize) {
                ...
                group.subgraphs = std::move(newSubgraphs);
            } else {
                if constexpr (verbose) {
    ...
    SubgraphSchedulerInput<GraphT, ConstrGraphT> PrepareSubgraphSchedulingInput(
        ...
        result.maxNumProcessors.resize(isomorphicGroups.size());
        result.requiredProcTypes.resize(isomorphicGroups.size());
        ...
        for (const auto &group : isomorphicGroups) {
            result.maxNumProcessors[coarseNodeIdx] = static_cast<unsigned>(group.Size() * group.subgraphs[0].size());
            ...
            result.requiredProcTypes[coarseNodeIdx].assign(numProcTypes, 0);
                ...
                for (const auto &vertex : subgraph) {
                    ...
                    for (unsigned j = 0; j < numProcTypes; ++j) {
                        if (originalInstance.IsCompatibleType(vertexType, j)) {
                            result.requiredProcTypes[coarseNodeIdx][j] += vertexWork;
                        }
                    }
                }
            }
        ...
        coarser_util::ConstructCoarseDag(
            originalInstance.GetComputationalDag(), result.instance.GetComputationalDag(), contractionMap);
    ...
    void ScheduleIsomorphicGroup(const BspInstance<GraphT> &instance,
        ...
        for (size_t groupIdx = 0; groupIdx < isomorphicGroups.size(); ++groupIdx) {
            ...
            const auto &procsForGroup = subSched.nodeAssignedWorkerPerType[groupIdx];
            ...
            bool useTrimmedScheduler = subSched.wasTrimmed[groupIdx] && minNonZeroProcs > 1 && allowUseTrimmedScheduler_;
            ...
            for (VertexIdxT<GraphT> i = 0; i < static_cast<VertexIdxT<GraphT>>(group.subgraphs.size()); ++i) {
                ...
                if (i == 0) {    // The first subgraph is the representative itself
                    ...
                    for (const auto &[hash, rep_orbit_nodes] : repHasher.GetOrbits()) {
                        ...
                        for (size_t k = 0; k < rep_orbit_nodes.size(); ++k) {
                            ...
                            currentVertexToRepLocalIdx[currentSubgraphVerticesSorted[currentOrbitNodes[k]]]
                                = static_cast<VertexIdxT<ConstrGraphT>>(rep_orbit_nodes[k]);
                        }
                    }
                }
