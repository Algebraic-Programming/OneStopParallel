...
struct BwdMerkleNodeHashFunc {
    MerkleHashComputer<GraphT, UniformNodeHashFunc<VertexIdxT<GraphT>>, false> bwMerkleHash;

    BwdMerkleNodeHashFunc(const GraphT &graph) : bwMerkleHash(graph) {}

    std::size_t operator()(const VertexIdxT<GraphT> &v) const { return bwMerkleHash.GetVertexHash(v); }
};

template <typename GraphT>
struct PrecomBwdMerkleNodeHashFunc {
    MerkleHashComputer<GraphT, VectorNodeHashFunc<VertexIdxT<GraphT>>, false> bwMerkleHash;

    PrecomBwdMerkleNodeHashFunc(const GraphT &graph, const std::vector<std::size_t> &nodeHashes)
        : bwMerkleHash(graph, nodeHashes) {}
