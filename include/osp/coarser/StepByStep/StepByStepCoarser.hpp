...
class StepByStepCoarser : public CoarserGenContractionMap<GraphT, GraphT> {
  ...
  public:
    ...
    struct EdgeToContract {
        ...
        VWorkwT<GraphT> workWeight;
        VCommwT<GraphT> commWeight;
  ...
  private:
    ...
    COARSENING_STRATEGY coarseningStrategy_ = COARSENING_STRATEGY::EDGE_BY_EDGE;
    PROBLEM_TYPE problemType_ = PROBLEM_TYPE::SCHEDULING;
  ...
  public:
    ...
    void SetCoarseningStrategy(COARSENING_STRATEGY strategy) { coarseningStrategy_ = strategy; }
...
std::vector<VertexIdxT<GraphT>> StepByStepCoarser<GraphT>::GenerateVertexContractionMap(const GraphT &dagIn) {
    ...
    if (coarseningStrategy_ == COARSENING_STRATEGY::EDGE_BY_EDGE) {
        // Init edge weights
        for (VertexIdx node = 0; node < n; ++node) {
    ...
    if (problemType_ == PROBLEM_TYPE::PEBBLING) {
...
std::vector<typename StepByStepCoarser<GraphT>::EdgeToContract> StepByStepCoarser<GraphT>::CreateEdgeCandidateList() const {
    ...
    for (auto it = contractable_.cbegin(); it != contractable_.cend(); ++it) {
        if (problemType_ == PROBLEM_TYPE::PEBBLING && IncontractableForPebbling(it->first)) {
            continue;
        }
...
std::pair<VertexIdxT<GraphT>, VertexIdxT<GraphT>> StepByStepCoarser<GraphT>::PickEdgeToContract(
    ...
    VWorkwT<GraphT> limitCardinality = candidates[limit].workWeight;
    while (limit < candidates.size() - 1 && candidates[limit + 1].workWeight == limitCardinality) {
        ++limit;
    }
    ...
    for (unsigned idx = 1; idx <= limit; ++idx) {
        if (candidates[idx].commWeight > candidates[best].commWeight) {
            best = idx;
        }
    }
...
std::vector<std::pair<VertexIdxT<GraphT>, VertexIdxT<GraphT>>> StepByStepCoarser<GraphT>::ClusterCoarsen() const {
    ...
    for (VertexIdx node = 0; node < gFull_.NumVertices(); ++node) {
        ...
        for (VertexIdx pred : gCoarse_.Parents(node)) {
            ...
            if (problemType_ == PROBLEM_TYPE::PEBBLING && IncontractableForPebbling(std::make_pair(pred, node))) {
                continue;
            }
        ...
        for (VertexIdx succ : gCoarse_.Children(node)) {
            ...
            // check viability for pebbling
            if (problemType_ == PROBLEM_TYPE::PEBBLING && IncontractableForPebbling(std::make_pair(node, succ))) {
                continue;
            }
...
void StepByStepCoarser<GraphT>::CoarsenForPebbling(const GraphT &dagIn,
                                                   GraphT &coarsenedDag,
                                                   std::vector<VertexIdxT<GraphT>> &newVertexId) {
    problemType_ = PROBLEM_TYPE::PEBBLING;
    coarseningStrategy_ = COARSENING_STRATEGY::EDGE_BY_EDGE;
