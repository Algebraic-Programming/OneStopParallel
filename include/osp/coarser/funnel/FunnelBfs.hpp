...
class FunnelBfs : public CoarserGenExpansionMap<GraphTIn, GraphTOut> {
  public:
    ...
    struct FunnelBfsParameters {
        ...
        bool useApproxTransitiveReduction;

        VWorkwT<GraphTIn> maxWorkWeight;
        VMemwT<GraphTIn> maxMemoryWeight;
  ...
  private:
    ...
    void RunInContraction(const GraphTIn &graph, std::vector<std::vector<VertexIdxT<GraphTIn>>> &partition) {
        ...
        const std::unordered_set<EdgeDescT<GraphTIn>> edgeMask = parameters_.useApproxTransitiveReduction
        ...
        for (auto revTopIt = topOrder.rbegin(); revTopIt != topOrder.crend(); revTopIt++) {
            ...
            while ((not vertexProcessingFifo.empty()) || (not nextVertexProcessingFifo.empty())) {
                if (vertexProcessingFifo.empty()) {
                    ...
                    depthCounter++;
                    if (depthCounter > parameters_.maxDepth) {
                        break;
                    }
                }
                ...
                if (workWeightOfGroup + graph.VertexWorkWeight(activeNode) > parameters_.maxWorkWeight) {
                    continue;
                }

                if (memoryWeightOfGroup + graph.VertexMemWeight(activeNode) > parameters_.maxMemoryWeight) {
                    continue;
                }
                ...
                for (const auto &inEdge : InEdges(activeNode, graph)) {
                    if (parameters_.useApproxTransitiveReduction && (edgeMask.find(inEdge) != edgeMask.cend())) {
                        continue;
                    }
                    ...
                    if (childrenNotInGroup.find(par) != childrenNotInGroup.cend()) {
                        ...
                        if (parameters_.useApproxTransitiveReduction) {
        ...
        for (const auto &topNode : top_sort_view(graph)) {
            ...
            while ((not vertexProcessingFifo.empty()) || (not nextVertexProcessingFifo.empty())) {
                ...
                for (const auto &outEdge : OutEdges(activeNode, graph)) {
                    if (parameters_.useApproxTransitiveReduction && (edgeMask.find(outEdge) != edgeMask.cend())) {
                        continue;
                    }
