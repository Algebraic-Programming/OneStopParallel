...
VertexIdxT<GraphTIn> Sarkar<GraphTIn, GraphTOut>::SingleContraction(
    ...
    const std::vector<VertexIdxT<GraphTIn>> vertexPoset
        = params_.useTopPoset ? GetTopNodeDistance<GraphTIn, VertexIdxT<GraphTIn>>(graph) : GetBotPosetMap(graph);
    ...
    for (const VertexType &edgeSrc : graph.Vertices()) {
        for (const VertexType &edgeTgt : graph.Children(edgeSrc)) {
            ...
            if (graph.VertexWorkWeight(edgeSrc) + graph.VertexWorkWeight(edgeTgt) > params_.maxWeight) {
                continue;
            }
    ...
    VertexIdxT<GraphTIn> maxCorseningNum
        = graph.NumVertices() - static_cast<VertexIdxT<GraphTIn>>(static_cast<double>(graph.NumVertices()) * params_.geomDecay);
...
VertexIdxT<GraphTIn> Sarkar<GraphTIn, GraphTOut>::AllChildrenContraction(
    ...
    for (const VertexType &groupHead : graph.Vertices()) {
        ...
        if (combinedWeight > params_.maxWeight) {
            continue;
        }
        ...
        long savings = static_cast<long>(maxPath) - static_cast<long>(newMaxPath);
        if (savings + static_cast<long>(params_.leniency * static_cast<double>(maxPath)) >= 0) {
...
std::vector<std::vector<VertexIdxT<GraphTIn>>> Sarkar<GraphTIn, GraphTOut>::GenerateVertexExpansionMap(const GraphTIn &dag_in,
    ...
    switch (params_.mode) {
        case SarkarParams::Mode::LINES: {
            diff = SingleContraction(params_.commCost, dag_in, expansionMap);
        } break;

        case SarkarParams::Mode::FAN_IN_FULL: {
            diff = AllParentsContraction(params_.commCost, dag_in, expansionMap);
        } break;

        case SarkarParams::Mode::FAN_IN_PARTIAL: {
            diff = SomeParentsContraction(params_.commCost, dag_in, expansionMap);
        } break;

        case SarkarParams::Mode::FAN_OUT_FULL: {
            diff = AllChildrenContraction(params_.commCost, dag_in, expansionMap);
        } break;

        case SarkarParams::Mode::FAN_OUT_PARTIAL: {
            diff = SomeChildrenContraction(params_.commCost, dag_in, expansionMap);
        } break;

        case SarkarParams::Mode::LEVEL_EVEN: {
            diff = LevelContraction(params_.commCost, dag_in, expansionMap);
        } break;
        ...
        case SarkarParams::Mode::HOMOGENEOUS_BUFFER: {
            diff = HomogeneousBufferMerge(params_.commCost, dag_in, expansionMap);
        } break;
...
VertexIdxT<GraphTIn> Sarkar<GraphTIn, GraphTOut>::SomeChildrenContraction(
    ...
    for (const VertexType &groupHead : graph.Vertices()) {
        ...
        for (std::size_t i = 0U; i < admissbleChildrenGroups.size(); ++i) {
            ...
            if (addedWeight > params_.maxWeight) {
                break;
            }
...
VertexIdxT<GraphTIn> Sarkar<GraphTIn, GraphTOut>::LevelContraction(
    ...
    const VertexIdxT<GraphTIn> parity = params_.mode == SarkarParams::Mode::LEVEL_EVEN ? 0 : 1;
    ...
    for (VertexIdxT<GraphTIn> headLevel = minLevel + parity; headLevel < maxLevel; headLevel += 2) {
        ...
        for (std::vector<VertexType> &comp : components) {
            ...
            if (uf.GetWeightOfComponentByName(comp.at(0)) > params_.maxWeight) {
                continue;
            }
...
VertexIdxT<GraphTIn> Sarkar<GraphTIn, GraphTOut>::HomogeneousBufferMerge(
    ...
    if (params_.mode == SarkarParams::Mode::FAN_OUT_BUFFER || params_.mode == SarkarParams::Mode::HOMOGENEOUS_BUFFER) {
    ...
    if (params_.mode == SarkarParams::Mode::FAN_IN_BUFFER || params_.mode == SarkarParams::Mode::HOMOGENEOUS_BUFFER) {
    ...
    for (const VertexType &vert : graph.Vertices()) {
        if (graph.VertexWorkWeight(vert) > params_.smallWeightThreshold) {
            continue;
        }
