...
std::unique_ptr<Coarser<Graph_t_in, Graph_t_out>> get_coarser_by_name(const ConfigParser &,
    ...
    if (coarser_name == "hdagg") {
        ...
        coarser->set_work_threshold(coarser_algorithm.get_child("parameters").get_child("max_work_weight").get_value_optional<v_workw_t<Graph_t_in>>().value_or(std::numeric_limits<v_workw_t<Graph_t_in>>::max()));
        coarser->set_memory_threshold(coarser_algorithm.get_value_optional<v_memw_t<Graph_t_in>>().value_or(std::numeric_limits<v_memw_t<Graph_t_in>>::max()));
        coarser->set_communication_threshold(coarser_algorithm.get_child("parameters").get_child("max_communication_weight").get_value_optional<v_commw_t<Graph_t_in>>().value_or(std::numeric_limits<v_commw_t<Graph_t_in>>::max()));
