...
namespace osp {
    ...
    class SquashA : public CoarserGenExpansionMap<Graph_t_in, Graph_t_out> {
        private:
            ...
            std::vector<std::vector<vertex_idx_t<Graph_t_in>>> gen_exp_map_from_contractable_edges(const std::multiset< std::pair<edge_desc_t<Graph_t_in>, T>, CMP> &edge_weights, const std::vector<int> &poset_int_mapping, const Graph_t_in &dag_in) {
                ...
                Union_Find_Universe<vertex_idx_t<Graph_t_in>, vertex_idx_t<Graph_t_in>, v_workw_t<Graph_t_in>, v_memw_t<Graph_t_in>> connected_components;
                ...
                vertex_idx_t<Graph_t_in> num_nodes_aim = dag_in.num_vertices() - static_cast<vertex_idx_t<Graph_t_in>>( static_cast<double>(dag_in.num_vertices()) / params.geom_decay_num_nodes );
                ...
                while( num_nodes_decrease < num_nodes_aim && temperature_increase_iteration <= params.number_of_temperature_increases ) {
                    for (const auto& wt_edge : edge_weights) {
                        ...
                        if ( connected_components.get_weight_of_component_by_name( edge_source )
                            + connected_components.get_weight_of_component_by_name( edge_target ) > static_cast<double>(lower_third_wt) * temperature ) continue;
                ...
                vertex_idx_t<Graph_t_in> min_node_decrease = dag_in.num_vertices() - static_cast<vertex_idx_t<Graph_t_in>>( static_cast<double>(dag_in.num_vertices()) / std::pow(params.geom_decay_num_nodes, 0.25) );
    ...
    template<typename Graph_t_in, typename Graph_t_out>
    std::vector<std::vector<vertex_idx_t<Graph_t_in>>> SquashA<Graph_t_in, Graph_t_out>::generate_vertex_expansion_map(const Graph_t_in &dag_in) {
        static_assert(is_directed_graph_edge_desc_v<Graph_t_in>, "Graph_t_in must satisfy the directed_graph_edge_desc concept");
        static_assert(is_computational_dag_edge_desc_v<Graph_t_in>, "Graph_t_in must satisfy the is_computational_dag_edge_desc concept");
        ...
        if (params.mode == SquashAParams::Mode::EDGE_WEIGHT) {
            ...
            {
                std::vector<edge_desc_t<Graph_t_in>> contractable_edges = get_contractable_edges_from_poset_int_map<Graph_t_in>(poset_int_mapping, dag_in);
