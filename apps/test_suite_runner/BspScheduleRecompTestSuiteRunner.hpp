...
class BspScheduleRecompTestSuiteRunner : public AbstractTestSuiteRunner<IBspScheduleEval<ConcreteGraphT>, ConcreteGraphT> {
  ...
  protected:
    RETURN_STATUS ComputeTargetObjectImpl(const BspInstance<ConcreteGraphT> &instance,
        ...
        if (schedulerNames.find(algoName) != schedulerNames.end()) {
            ...
            RETURN_STATUS status = RunBspScheduler(this->parser_, algoConfig, *bspSchedule);
            ...
            RETURN_STATUS status = RunBspRecompScheduler(this->parser_, algoConfig, *bspRecompSchedule);
