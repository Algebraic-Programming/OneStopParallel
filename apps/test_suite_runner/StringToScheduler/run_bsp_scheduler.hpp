...
const std::set<std::string> get_available_bsp_scheduler_names() {
    return {"Serial",       "GreedyBsp",      "GrowLocal", "BspLocking", "Cilk",        "Etf",
            "GreedyRandom", "GreedyChildren", "Variance",  "MultiHC",    "LocalSearch", "Coarser", "FullILP", "MultiLevel"};;
}

template<typename Graph_t> 
std::unique_ptr<ImprovementScheduler<Graph_t>> get_bsp_improver_by_name(const ConfigParser &, const boost::property_tree::ptree &algorithm) {
...
get_base_bsp_scheduler_by_name(const ConfigParser &parser, const boost::property_tree::ptree &algorithm) {
    ...
    if (id == "Serial") {
        ...
        auto scheduler = std::make_unique<GreedyBspScheduler<Graph_t>>(max_percent_idle_processors, increase_parallelism_in_new_superstep);
        ...
        params.syncCostMultiplierMinSuperstepWeight = algorithm.get_child("parameters").get_child("syncCostMultiplierMinSuperstepWeight").get_value<v_workw_t<Graph_t>>();
        params.syncCostMultiplierParallelCheck = algorithm.get_child("parameters").get_child("syncCostMultiplierParallelCheck").get_value<v_workw_t<Graph_t>>();
        ...
        algorithm.get_child("parameters").get_child("mode").get_value<std::string>() == "SJF" ? scheduler->setMode(CilkMode::SJF) : scheduler->setMode(CilkMode::CILK);
        return scheduler;

    } else if (id == "Etf") {
        auto scheduler = std::make_unique<EtfScheduler<Graph_t>>();
        algorithm.get_child("parameters").get_child("mode").get_value<std::string>() == "BL_EST" ? scheduler->setMode(EtfMode::BL_EST) : scheduler->setMode(EtfMode::ETF);
        return scheduler;
        ...
        float max_percent_idle_processors = algorithm.get_child("parameters").get_child("max_percent_idle_processors").get_value<float>();
        bool increase_parallelism_in_new_superstep = algorithm.get_child("parameters").get_child("increase_parallelism_in_new_superstep").get_value<bool>();
        auto scheduler = std::make_unique<VarianceFillup<Graph_t>>(max_percent_idle_processors, increase_parallelism_in_new_superstep);
...
RETURN_STATUS run_bsp_scheduler(const ConfigParser &parser, const boost::property_tree::ptree &algorithm,
    ...
    if (id == "LocalSearch") {
        ...
        std::unique_ptr<ImprovementScheduler<Graph_t>> improver = get_bsp_improver_by_name<Graph_t>(parser, algorithm.get_child("parameters").get_child("improver"));
        ...
        std::unique_ptr<Coarser<Graph_t, boost_graph_t>> coarser = get_coarser_by_name<Graph_t, boost_graph_t>(parser, algorithm.get_child("parameters").get_child("coarser"));
        ...
        const auto status_coarse = run_bsp_scheduler(parser, algorithm.get_child("parameters").get_child("scheduler"), schedule_coarse);
