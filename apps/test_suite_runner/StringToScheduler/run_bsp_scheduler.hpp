...
std::unique_ptr<Scheduler<GraphT>> GetBaseBspSchedulerByName(const ConfigParser &parser,
    ...
    if constexpr (isConstructableCdagV<GraphT> || isDirectConstructableCdagV<GraphT>) {
        if (id == "MultiHC") {
            ...
            const unsigned timeLimit = parser.globalParams.get_child("timeLimit").get_value<unsigned>();
...
RETURN_STATUS RunBspScheduler(const ConfigParser &parser,
    ...
    if (id == "LocalSearch") {
        RETURN_STATUS status = RunBspScheduler(parser, algorithm.get_child("parameters").get_child("scheduler"), schedule);
        ...
        if (algorithm.get_child("parameters").get_child("use_initial_solution").get_value<bool>()) {
            ...
            RETURN_STATUS status = RunBspScheduler(
        ...
        if (statusCoarse != RETURN_STATUS::OSP_SUCCESS and statusCoarse != RETURN_STATUS::BEST_FOUND) {
            return statusCoarse;
        }
