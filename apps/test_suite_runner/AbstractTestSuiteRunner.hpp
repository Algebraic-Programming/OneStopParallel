...
class AbstractTestSuiteRunner {
  protected:
    ...
    virtual bool ParseCommonConfig() {
        try {
            ...
            timeLimitSeconds_ = parser_.globalParams.get_child("timeLimit").get_value<unsigned>();
            writeTargetObjectToFile_ = parser_.globalParams.get_child("outputSchedule").get_value_optional<bool>().value_or(false);

            graphDirPath_ = parser_.globalParams.get_child("graphDirectory").get_value<std::string>();
            if (graphDirPath_.substr(0, 1) != "/") {
                graphDirPath_ = executableDir_ + graphDirPath_;
            }

            machineDirPath_ = parser_.globalParams.get_child("archDirectory").get_value<std::string>();
            if (machineDirPath_.substr(0, 1) != "/") {
                machineDirPath_ = executableDir_ + machineDirPath_;
            }

            if (writeTargetObjectToFile_) {
                outputTargetObjectDirPath_ = parser_.globalParams.get_child("scheduleDirectory").get_value<std::string>();
            ...
            logFilePath_ = parser_.globalParams.get_child("outputLogFile").get_value<std::string>();
            if (logFilePath_.substr(0, 1) != "/") {
                logFilePath_ = executableDir_ + logFilePath_;
            }

            statisticsOutputFilePath_ = parser_.globalParams.get_child("outputStatsFile").get_value<std::string>();
  ...
  public:
    ...
    int Run(int argc, char *argv[]) {
        ...
        try {
            for (const auto &item : parser_.globalParams.get_child("activeStatisticModules")) {
        ...
        for (const auto &machineEntry : std::filesystem::recursive_directory_iterator(machineDirPath_)) {
            ...
            for (const auto &graphEntry : std::filesystem::recursive_directory_iterator(graphDirPath_)) {
                ...
                for (auto &algorithmConfigPair : parser_.scheduler) {
                    ...
                    RETURN_STATUS execStatus = ComputeTargetObjectImpl(bspInstance, targetObject, algoConfig, computationTimeMs);

                    if (execStatus != RETURN_STATUS::OSP_SUCCESS && execStatus != RETURN_STATUS::BEST_FOUND) {
                        if (execStatus == RETURN_STATUS::ERROR) {
                            logStream_ << "Error computing with " << currentAlgoName << "." << std::endl;
                        } else if (execStatus == RETURN_STATUS::TIMEOUT) {
