...
inline const char *ModeTag(SCHEDULE_NODE_PERMUTATION_MODES m) {
    switch (m) {
        case NO_PERMUTE:
            return "NoPermute_";
...
class BspSptrsvStatsModule : public IStatisticModule<TargetObjectType> {
  public:
    explicit BspSptrsvStatsModule(SCHEDULE_NODE_PERMUTATION_MODES mode = NO_PERMUTE) : mode_(mode) {}
    ...
    std::map<std::string, std::string> RecordStatistics(const TargetObjectType &schedule, std::ofstream &) const override {
        ...
        if constexpr (std::is_same_v<TargetObjectType, osp::BspSchedule<osp::SparseMatrixImp<int32_t>>>
            ...
            long long geomMean = static_cast<long long>(std::exp(totalLog / runs));

            // Standard deviation
            double mean = std::accumulate(timesNs.begin(), timesNs.end(), 0.0) / runs;
            double sqSum = std::accumulate(timesNs.begin(), timesNs.end(), 0.0, [mean](double acc, long long val) {
                double diff = static_cast<double>(val) - mean;
                return acc + diff * diff;
            });
            long long stddev = static_cast<long long>(std::sqrt(sqSum / runs));

            // Quartiles
            std::sort(timesNs.begin(), timesNs.end());
            long long q25 = timesNs[runs / 4];
            long long q75 = timesNs[3 * runs / 4];
  ...
  private:
    SCHEDULE_NODE_PERMUTATION_MODES mode_;
