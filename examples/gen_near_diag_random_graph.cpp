...
int main(int argc, char *argv[]) {
    ...
    if (argc > 4) {
        num_graphs = std::atoi(argv[4]);
    }

    // std::cout << "Vert: " << num_vert << " prob: " << prob << " bandwidth: " << bandwidth << " graphs: " << num_graphs << std::endl;
    ...
    for (size_t i = 0; i < num_graphs; i++) {
        ...
        ComputationalDag graph = near_diag_random_graph(num_vert, bandwidth, prob).ConvertToNewDAG();
