...
@recipe function sankey(
    ...
    max_width=0.2,
    node_widths=nothing,
    bsp_position_force=Pair{Int,Int}(0,0),
)
    ...
    x, y, mask = ((bsp_position_force.first > 0) && (bsp_position_force.second > 0)) ? bsp_layout!(g, bsp_position_force) : sankey_layout!(g, force_layer, force_order, force_equal_layers)
    perm = sortperm(y, rev=true)

    if ((bsp_position_force.first > 0) && (bsp_position_force.second > 0))
    ...
    if node_widths !== nothing
        mxwidth = maximum(node_widths)
        node_widths .*= max_width / mxwidth
    else
        node_widths = ones(length(unique([s.src; s.dst]))) * max_width
    end
    ...
    for (i, v) in enumerate(vertices(g))
        ...
        if !(mask[i])
            @series begin
                ...
                [x[i]-node_widths[i]/2, x[i]+node_widths[i]/2, x[i]+node_widths[i]/2, x[i]-node_widths[i]/2], [y[i]-h, y[i]-h, y[i]+h, y[i]+h]
            end
            ...
            for (j, w) in enumerate(vertices(g))
                if has_edge(g, v, w)
                    ...
                    edge_it = Edge(v, w)
                    h_edge = get_prop(g, edge_it, :weight) / (m) * max_height

                    sankey_y = Float64[]
                    x_start = x[i] + node_widths[i]/2
                    k = j
                    l = i
                    while mask[k]
                        y_dst = y[k] + vw[k] / (m) * max_height - dst_offsets[k, l]
                    ...
                    y_dst = y[k] + vw[k] / (m) * max_height - dst_offsets[k, l]
                    x_coords = range(0, 1, length=length(x_start:0.01:x[k]-node_widths[k]/2))
                    y_coords = remap(1 ./ (1 .+ exp.(6 .* (1 .- 2 .* x_coords))), y_src, y_dst)
                    append!(sankey_y, y_coords)
                    sankey_x = range(x[i]+node_widths[i]/2, x[k]-node_widths[k]/2, step=0.01)
            ...
            if label_position !== :legend
                ...
                elseif label_position === :right ||
                    ...
                    x[i] + node_widths[i]/2 + 0.05, :left
                elseif label_position === :left ||
                        (label_position === :inside && x[i] == maximum(x))
                    x[i] - node_widths[i]/2 - 0.05, :right
                else
                    error("label_position :$label_position not supported")
                end
...
function sankey_layout!(g, force_layer, force_order, force_equal_layers)
    xs, ys, paths = solve_positions(
        Zarate(), g, force_layer=force_layer, force_order=force_order, force_equal_layers=force_equal_layers,
    )
...
function bsp_layout!(g, bsp_position_force)
    n_proc, n_step = bsp_position_force
